...
修复前稳定红灯：16 个同 credential 并发 caller 命中 `ListAvailableProfiles` 16 次，断言期望 1 次，实际为 16。测试使用 `127.0.0.1:0` 的 Axum endpoint 和真实 reqwest transport，不访问官方 Account Runtime，也不替换 provider 内部函数。
...
`src/local_upstream_impl/provider.rs` 现在使用进程内、按 credential 隔离的异步 gate：
...
- `account-runtime_upstream_base_url` 现在与其他 Account Runtime endpoint 一样只覆盖 transport destination；逻辑 AWS Host header 仍按 region 生成。
...
cargo +1.92.0 test 'account-runtime::provider::tests::' -- --nocapture
```
...
debug test binary: target/debug/deps/account_runtime-a8445619ee052375
...
- fake endpoint 覆盖真实 HTTP 协议和 provider 调用链，但不是官方 external IdP/Account Runtime 账号验证。
