# Account Runtime Thinking Protocol Cross-Check - 2026-07-17
...
This review compares the current `kiro.rs` reasoning design with the installed official Account Runtime
...
2. the installed official Account Runtime bundle;
...
| --- | --- | --- |
| Official Account Runtime app/agent | app `1.0.89`, agent `1.0.165`, bundle SHA-256 `8e6152abd4ae223f0efb1d7642005619432b3c710ec0ca7e107dadb2463aa7bb` | dynamic reasoning schema, signed/redacted history, signature-invalid retry |
| `chaogei/Account Runtime-account-manager` | `447adcdb468157312621b1f09448278bd9bca748` | dynamic effort mapping, reasoning history and compatibility retry |
...
remains `chaogei/Account Runtime-account-manager` and its release notes:

- <https://github.com/chaogei/Account Runtime-account-manager>
- <https://github.com/chaogei/Account Runtime-account-manager/releases>
...
that multi-turn history now drops `reasoningContent` because Account Runtime rejects that field in request
...
## Official Account Runtime evidence
...
signed history when that model ID matches the current formatted model. If Account Runtime returns the exact
...
### Account Runtime-account-manager
...
- `translator.ts` and `kiroApi.ts` map current-turn / response signed text and redacted content into
  Account Runtime/Anthropic-compatible reasoning content.
...
- Current local source intentionally drops history `reasoningContent` before sending request history
  to Account Runtime, citing backend `400 Improperly formed request`. That may be a valid compatibility choice
  for that project, but it cannot close this repository's signed/redacted history contract; our
  implementation still needs exact frozen Account Runtime wire and signature-invalid fixtures.
...
- Multiple redacted blocks are concatenated, although the observed Account Runtime history field is a strict
...
- It does not provide evidence for signed Account Runtime request-history round trips or the exact
...
- Capture the frozen candidate's final CLI and IDE-family Account Runtime wire bodies for all effort values.
