...
- 要求 `ACCOUNT_RUNTIME_BINARY` 为仓库外复制的冻结候选，不接受 `target/debug` 或 `target/release` 直接产物。
- 要求 `ACCOUNT_RUNTIME_VALIDATION_ARTIFACT_DIR` 为仓库外 owned artifact root。
- 要求调用者提供 `ACCOUNT_RUNTIME_E01_E02_POSTGRES_URL_TEMPLATE`，模板必须包含一次 `{database}`，且只能指向 loopback PostgreSQL。
- 要求调用者提供 `ACCOUNT_RUNTIME_E01_E02_POSTGRES_DATABASES`，数量必须等于 `modes × rounds`，名称必须是 caller-owned `account-runtime_e0102_*`。
- 要求调用者提供 `ACCOUNT_RUNTIME_E01_E02_REDIS_URL`，只能是 loopback `redis://`，DB 必须是 `1..15`，不允许 auth/query/fragment。
- 要求调用者提供 `ACCOUNT_RUNTIME_E01_E02_REDIS_PREFIX`，不能包含生产/共享 `account_runtime:local`。
- dynamic runner 不再执行 Docker、不再 `FLUSHDB`、不再 `CREATE DATABASE`；每个 case 使用独立 Redis `keyPrefix`，只清理该 prefix。
- 支持 `ACCOUNT_RUNTIME_E01_E02_VALIDATE_ONLY=1`，可在无 PG/Redis 网络访问前验证输入、binary/artifact 路径、database 数量和安全边界。
...
| 通过 Docker `psql CREATE DATABASE` 创建 case DB | 不创建数据库，要求调用者预创建 `account-runtime_e0102_*` DB |
...
- Redis DB0、非 loopback Redis、Redis port `9022`、共享 `account_runtime:local` prefix 全部在 runtime 前拒绝。
...
ACCOUNT_RUNTIME_E01_E02_POSTGRES_DATABASES='account-runtime_e0102_run_01,account-runtime_e0102_run_02,...' \
...
ACCOUNT_RUNTIME_E01_E02_POSTGRES_DATABASES='account-runtime_e0102_bal_01,account-runtime_e0102_bal_02,account-runtime_e0102_bal_03,account-runtime_e0102_weight_01,account-runtime_e0102_weight_02,account-runtime_e0102_weight_03' \
...
```
