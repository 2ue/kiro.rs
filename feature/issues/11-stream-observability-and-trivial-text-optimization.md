...
Status: `implementation-present / unified-candidate-cli-and-load-gates-pending`
...
修复前 usage 无法说明 EOF 前最后看到了哪些上游事件，`suspectedIntentPreambleEndTurn` 只有 bool，孤立 `.`/空白又可能在 tool_use 前作为可见正文下发。它们分别表现为“成功却像截断”“模型说要做但结束”“工具调用前显示无意义标点”，同时缺少足够证据区分协议、模型行为与 UI 展示。

## 根因

stream state 只保留生成下游事件所需的即时状态，没有固定上限的 terminal event 摘要；intent preamble 使用过于粗的二值启发式；trivial text 按单 delta 直接下发，而正常标点又可能是被拆分正文的一部分，因此不能用无状态过滤安全删除。

## 复现方案

fake upstream 分别发送：无 `COMPLETED` 的正常 EOF、显式 completed、短行动意图后 end_turn、孤立 `.` 后 tool_use、`3` + `.` + `14` 正常正文、thinking-only 与无 text/tool。每种 5 轮并枚举 transport chunk partition；真实 CLI 再运行 normal/tool/thinking 长会话，对齐 JSONL、SSE 和 usage。
...
事件 ring buffer 和启发式字段只能改善观测，不能单独证明 silent truncation；trivial buffer 仍需逐 chunk、thinking/tool boundary 和真实 CLI 验证。回滚可关闭 trivial filtering 或新增诊断展示，但不能恢复伪造 completion、无上限事件正文记录或按单个 `.` 无状态删除。语言串台、invalid model 调度与 thinking-only 空格仍由各自专题验收。
