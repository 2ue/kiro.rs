...
Return the exact invalid-bearer response from fake Account Runtime API and MCP endpoints for concurrency 1, 8, and 32. Record inference/MCP hits, refresh hits, credential ID, token generation, storage revision, health state, and request-level inference/auxiliary budgets.
