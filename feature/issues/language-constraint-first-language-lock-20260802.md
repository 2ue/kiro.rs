...
Status: `analysis-planned / reproduction-pending / implementation-not-authorized`
...
- 语言检测、语言约束提示词生成和 prompt steering 的入口；
- system prompt 合并、会话历史、压缩/裁剪和重试路径；
- `/v1`、`/cc`、`/ha`、`/na` 及自定义路径的配置规则；
- 流式和 `count_tokens` 是否共享同一语言状态；
- 是否有全局或进程级缓存错误复用到不同会话。
