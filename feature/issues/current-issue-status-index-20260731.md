...
Current inventory from the 2026-08-04 external-pool retry/cooldown update:
...
| External pool direct/model/retry behavior | [external-pool-direct-model-retry-20260804.md](external-pool-direct-model-retry-20260804.md) | `analysis-updated / cross-host-evidence-matrix-recorded / target-semantics-confirmed / p0-implemented / retry-cooldown-focused-verified / follow-ups-open` | 159/170/142 已按“入口、路由、外部尝试、本地尝试、模型字段”补齐对照；本轮确认 `usage` 明细保存的是请求决策轨迹，不是完整运行时配置快照，因此 `Fallback 原因 local_no_credentials` 只能证明这条请求走过预检分支，不能单独证明全局直连配置状态。当前证据没有证明外部直连隐式回本地，用户看到的本地错误样本来自 `/dfcache/onlylocal` 仅本地入口。P0 已修复并 focused 验证：外部池候选选择按路由配置、启用状态、优先级/容量/冷却和模型支持执行，`请求正文模式` 只在选中池后决定 body 处理；Raw 入口可在 Raw 池 502 后重选标准处理池；外部池默认补 `anthropic-version: 2023-06-01`。同池重试、跨池预算分离、“跨池重试状态码”“网络错误跨池重试”“协议错误跨池重试”“Retry-After 驱动的池级冷却”“连续瞬态失败冷却上浮”和“清除冷却”已通过本地隔离 PostgreSQL/Redis + fake upstream 验证。外部失败后的 `local rescue` 现仅在本地优先 fallback 且 fresh 本地状态为 `Ready`、仍有可调度容量时允许；直连、无本地凭证和其他终态本地不可调度状态不回本地。后续仍开放生产复发观察、候选筛选原因和模型字段可观测性，多实例冷却竞态继续由更大调度门禁覆盖 |
| Scheduler architecture | [scheduler-architecture-analysis-purpose-and-plan.md](scheduler-architecture-analysis-purpose-and-plan.md) | `analysis-complete / target-design-proposed / decision-proposed / compliance-matrix-recorded / implementation-not-authorized` | Source-verified local/external/fallback/rescue state machine, queue/capacity/cooldown/retry matrix, `sub2api` comparison and configuration regrouping are recorded. The durable [target state-machine and test contract](../../docs/plantree/plans/rust-runtime-scheduler-stabilization/topics/scheduler-target-state-machine-and-test-contract.md) now makes route modes, allowed/forbidden transitions, error actions, shared deadline, health-aware priority behavior and sustained fake-upstream validation explicit. Proposed [Decision 001](../../docs/plantree/plans/rust-runtime-scheduler-stabilization/decisions/001-local-external-scheduler-target-contract.md), [current compliance matrix](../../docs/plantree/plans/rust-runtime-scheduler-stabilization/topics/scheduler-target-compliance-matrix.md) and [sustained validation plan](../../docs/plantree/plans/rust-runtime-scheduler-stabilization/topics/sustained-scheduling-validation.md) still separate focused evidence from structural non-conformance and unexecuted L1–L5 gates; implementation still requires decision acceptance and staged execution |
