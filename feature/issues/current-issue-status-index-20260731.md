...
current scoped patch batch once its validation completed. That batch now has
passing release evidence in [Final release gate - 2026-08-01](../evidence/final-release-gate-20260801.md)
and is treated as `GO` for `v0.0.126` after the dedicated Cargo version
bump/tag validation. The `NO-GO` rows below are broader production,
...
| Scheduler architecture | [scheduler-architecture-analysis-purpose-and-plan.md](scheduler-architecture-analysis-purpose-and-plan.md) | `analysis-planned / read-only-source-inventory-started` | Complete route planner / capacity ledger design and turn the architecture analysis into executable plan state |
