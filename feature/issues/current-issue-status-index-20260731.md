...
| Usage accounting / downstream standard fields | [downstream-usage-standard-field-over-1m-20260731.md](downstream-usage-standard-field-over-1m-20260731.md) | `analysis-recorded / production-evidence-collected / standard-field-guard-implemented / focused-tests-passed / scoped-release-gate-passed / production-recurrence-pending` | Known code residuals are now focused-tested: no-reportedUsage local prompt-cache standard cache fields are capped, including `account_runtime_tool`; local credential and external pool failure records keep request estimates in diagnostics and zero downstream-standard fields. 2026-08-01 scoped release gate passed; remaining gates are dashboard/API rollup distinction, production recurrence, and broader load validation |
...
| Stream terminal / external pool pre-output retry | [stream-terminal-errors-and-precommit-retry.md](stream-terminal-errors-and-precommit-retry.md) | `focused-router-and-release-2mib-unit-passed / external-pool-pre-output-stream-retry-implemented / focused-fake-http-and-routing-validated / normal-routing-rerun-passed-20260807 / frozen-cli-load-passed-20260807 / released-v0.0.134 / production-rollout-observation-pending` | 2026-08-06 `req_01KaWrDY5oZkY13XQqdJB9PH` and yuenan/yuenan-1 sampling show external-direct stream `HTTP 200 -> message_start -> error` before content/thinking/tool output. The implementation now buffers protocol-only SSE before downstream commit and can retry another external pool on pre-output error/read/idle/EOF, with global default plus per-pool override, PostgreSQL/admin/UI wiring, and fake-upstream validation. 2026-08-07 rerun passed normal-output/routing coverage for external stream/non-stream, local stream/non-stream, external direct stream/non-stream, local-first fallback, direct no-local-rescue, fresh local Ready, Redis degraded classification, route config authority, Rust check/fmt, UI/admin-ui build, docs, diff and artifact inventory. Final frozen candidate also passed real Claude Code CLI `2.1.221` fake-upstream gates (`bare 20/20`, long-session `110 turns`, thinking-wire rerun `60/60`) and L3/L4/L5 load/chaos (`9/9`, `12/12`, `900s` soak `6820/6820` with RSS/FD recovery). `v0.0.134` was published by GitHub Actions `Publish Docker Images #166`; production rollout observation and `yuenan` / `yuenan-1` recurrence checks remain post-release |
