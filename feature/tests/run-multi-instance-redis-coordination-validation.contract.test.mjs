...
const LIVE_NONEMPTY_URL = String(
  process.env.ACCOUNT_RUNTIME_MULTI_INSTANCE_CONTRACT_NONEMPTY_REDIS_URL || '',
).trim()

function runRunner(environment) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'account-runtime-multi-instance-contract-'))
...
for (let round = 1; round <= 3; round += 1) {
  test(`missing Redis URL rejects before Cargo round ${round}`, async () => {
    const result = await runRunner({ ACCOUNT_RUNTIME_TEST_REDIS_ISOLATED: '1' })
    assert.notEqual(result.code, 0)
    assert.match(result.stderr, /ACCOUNT_RUNTIME_MULTI_INSTANCE_REDIS_URL is required/)
  ...
  test(`isolation marker must be exact 1 round ${round}`, async () => {
    const result = await runRunner({
      ...
      ACCOUNT_RUNTIME_TEST_REDIS_ISOLATED: round === 1 ? '0' : 'true',
    })
    assert.notEqual(result.code, 0)
    assert.match(result.stderr, /ACCOUNT_RUNTIME_TEST_REDIS_ISOLATED=1 is required/)
    assert.equal(result.cargoCalled, false)
  })

  test(`DB0 rejects before Redis and Cargo round ${round}`, async () => {
    const result = await runRunner({
      ACCOUNT_RUNTIME_MULTI_INSTANCE_REDIS_URL: 'redis://127.0.0.1:26379/0',
  ...
  test(`protected 9022 rejects without probing it round ${round}`, async () => {
    const result = await runRunner({
      ACCOUNT_RUNTIME_MULTI_INSTANCE_REDIS_URL: 'redis://127.0.0.1:9022/15',
      ACCOUNT_RUNTIME_TEST_REDIS_ISOLATED: '1',
    })
    assert.notEqual(result.code, 0)
...
test('caller-confirmed nonempty Redis rejects before Cargo', {
  skip: LIVE_NONEMPTY_URL ? false : 'set ACCOUNT_RUNTIME_MULTI_INSTANCE_CONTRACT_NONEMPTY_REDIS_URL to opt in',
}, async () => {
  ...
  const result = await runRunner({
    ACCOUNT_RUNTIME_MULTI_INSTANCE_REDIS_URL: LIVE_NONEMPTY_URL,
    ACCOUNT_RUNTIME_TEST_REDIS_ISOLATED: '1',
  })
  assert.notEqual(result.code, 0)
