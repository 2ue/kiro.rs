...
const ROUNDS = Number.parseInt(process.env.ACCOUNT_RUNTIME_F06_ROUNDS || '3', 10)
const { binary: BINARY, artifactRoot: ARTIFACT_ROOT } = resolveRuntimeValidationPaths(ROOT)
const POSTGRES_URL = requiredEnvironment('ACCOUNT_RUNTIME_F06_POSTGRES_URL')
const REDIS_URL = requiredEnvironment('ACCOUNT_RUNTIME_F06_REDIS_URL')
const REDIS_PREFIX = requiredEnvironment('ACCOUNT_RUNTIME_F06_REDIS_PREFIX')
const VALIDATE_ONLY = process.env.ACCOUNT_RUNTIME_F06_VALIDATE_ONLY === '1'
...
const MALFORMED_CREDENTIAL_VARIANTS = [
  { id: 'empty_key', plainFile: true, fields: () => ({ apiKey: '|us-east-1' }) },
  { id: 'multiple_pipe', plainFile: true, fields: (secret) => ({ apiKey: `${secret}|us-east-1|extra` }) },
  { id: 'region_whitespace', plainFile: true, fields: (secret) => ({ apiKey: `${secret}|us east-1` }) },
  { id: 'region_control', plainFile: true, fields: (secret) => ({ apiKey: `${secret}|us-east-1\nextra` }) },
  { id: 'region_host_unsafe', plainFile: true, fields: (secret) => ({ apiKey: `${secret}|us-east-1.example` }) },
  { id: 'explicit_region_whitespace', plainFile: false, fields: (secret) => ({ apiKey: secret, region: 'us east-1' }) },
  { id: 'explicit_auth_region_control', plainFile: false, fields: (secret) => ({ apiKey: secret, authRegion: 'us-east-1\nextra' }) },
  { id: 'explicit_api_region_host_unsafe', plainFile: false, fields: (secret) => ({ apiKey: secret, apiRegion: 'us-east-1.example' }) },
]
...
if (!Number.isInteger(ROUNDS) || ROUNDS < 3 || ROUNDS > 10) {
  throw new Error('ACCOUNT_RUNTIME_F06_ROUNDS must be an integer between 3 and 10')
}
...
function validateInputs() {
  ...
  if (!['postgres:', 'postgresql:'].includes(postgres.protocol)) {
    throw new Error('ACCOUNT_RUNTIME_F06_POSTGRES_URL must use PostgreSQL')
  }
  if (!['127.0.0.1', 'localhost', '::1'].includes(postgres.hostname)) {
    throw new Error('ACCOUNT_RUNTIME_F06_POSTGRES_URL must target loopback')
  }
  if (Number(postgres.port || 5432) === 9022) throw new Error('port 9022 is protected')
  if (postgres.hash) throw new Error('ACCOUNT_RUNTIME_F06_POSTGRES_URL must not contain a fragment')
  for (const key of postgres.searchParams.keys()) {
    if (key !== 'sslmode') throw new Error(`ACCOUNT_RUNTIME_F06_POSTGRES_URL contains unsupported query parameter ${key}`)
  }
  const postgresDatabase = decodeURIComponent(postgres.pathname.replace(/^\//, ''))
  if (!/^account_runtime_f06_[a-z0-9_]{3,80}$/.test(postgresDatabase)) {
    throw new Error('ACCOUNT_RUNTIME_F06_POSTGRES_URL must name a caller-owned account_runtime_f06_* database')
  }

  const redis = new URL(REDIS_URL)
  if (redis.protocol !== 'redis:') throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must use redis://')
  if (redis.username || redis.password) throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must not contain Redis auth material')
  if (!['127.0.0.1', 'localhost', '::1'].includes(redis.hostname)) {
    throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must target loopback')
  }
  if (Number(redis.port || 6379) === 9022) throw new Error('port 9022 is protected')
  if (redis.search || redis.hash) throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must not contain query or fragment data')
  const dbText = redis.pathname.replace(/^\//, '')
  if (!/^\d+$/.test(dbText)) throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must name a Redis database')
  const redisDatabase = Number(dbText)
  if (!Number.isSafeInteger(redisDatabase) || redisDatabase < 1 || redisDatabase > 15) {
    throw new Error('ACCOUNT_RUNTIME_F06_REDIS_URL must use an isolated nonzero database in 1..15')
  }
  if (REDIS_PREFIX.includes('account_runtime:local')) {
    throw new Error('ACCOUNT_RUNTIME_F06_REDIS_PREFIX must be a caller-owned temporary prefix')
  }
  if (!/^[a-z0-9][a-z0-9:._-]{7,95}$/.test(REDIS_PREFIX)) {
    throw new Error('ACCOUNT_RUNTIME_F06_REDIS_PREFIX has an invalid format')
  }
  redisTarget = {
    redis,
...
function expectedHost(kind, region) {
  if (kind === 'model_discovery') return `management.${region}.account-runtime.local`
  if (kind === 'inference') return `runtime.${region}.account-runtime.local`
  return `q.${region}.account-runtime.local`
}
...
function createFakeUpstream() {
  ...
  const server = http.createServer((request, response) => {
    ...
    if (kind === 'balance') {
      writeJson(response, 200, {
        subscriptionInfo: {
          subscriptionTitle: 'ACCOUNT RUNTIME F06 TEST',
...
function isolatedServiceConfig({ port, upstreamPort, redisKeyPrefix }) {
  return {
    ...
    defaultEndpoint: 'ide',
    upstreamBaseUrl: `http://127.0.0.1:${upstreamPort}/f06`,
...
function psql(sql, applicationName = 'account_runtime_f06_validation') {
...
function postgresTableExists(tableName) {
  const raw = psql(`SELECT to_regclass('public.${tableName}') IS NOT NULL;`, 'account_runtime_f06_table_probe')
  return raw === 't'
}
...
function assertInitialDatabaseOwnedAndSafe() {
  ...
  `, 'account_runtime_f06_initial_database_probe')
  ...
  `, 'account_runtime_f06_initial_credential_probe'), 10)
  ...
  if (activeCredentials !== 0) {
    throw new Error('ACCOUNT_RUNTIME_F06_POSTGRES_URL database must not contain active credentials before validation')
  }
...
function databaseCredentialState() {
  ...
  const raw = psql(`
    SELECT json_build_object(
      ...
      'apiKeyPresent', COALESCE(bool_and(COALESCE(data->>'apiKey', data->>'account_runtime_api_key') IS NOT NULL) FILTER (WHERE deleted_at IS NULL), false),
      'apiKeyContainsPipe', COALESCE(bool_or(COALESCE(data->>'apiKey', data->>'account_runtime_api_key') LIKE '%|%') FILTER (WHERE deleted_at IS NULL), false),
...
async function runMalformedCredentialFileCases({ mode, upstreamPort, fake }) {
  ...
  for (let round = 1; round <= 3; round += 1) {
    for (const variant of variants) {
      ...
      if (mode === 'json_file') {
        assert.ok(
          log.includes('invalid API-key import fields') || log.includes('invalid apiKey pipe format'),
...
async function runCase({ mode, round, upstreamPort, fake, latencies, statuses }) {
  ...
  if (mode === 'admin_api') {
    ...
    fs.writeFileSync(credentialsPath, `${JSON.stringify([{
      ...
      profileArn: `arn:account-runtime:wrong-region:123:profile/STALE`,
    }], null, 2)}\n`, { mode: 0o600 })
  } else {
  ...
  const duplicate = await timedRequest(`${baseUrl}/api/admin/credentials`, {
    method: 'POST',
    headers: adminHeaders(),
    body: JSON.stringify({ authMethod: 'api_key', apiKey: `${secret}|${region}` }),
  })
...
async function main() {
  ...
  PSQL = resolveExecutable(process.env.ACCOUNT_RUNTIME_PSQL_BINARY || 'psql')
  ...
  try {
    ...
    const report = {
      ...
      isolation: {
        ...
        realAccountRuntimeConfigured: false,
        realAccountRuntimeAccessObserved: false,
