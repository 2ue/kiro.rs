...
const FIXTURE_MODE = process.env.ACCOUNT_RUNTIME_THINKING_WIRE_FIXTURE_MODE
  || (process.env.ACCOUNT_RUNTIME_THINKING_WIRE_SIGNAL_FIXTURE === '1' ? 'signal_idle' : '')
...
if (FIXTURE_MODE && !FIXTURE_MODES.has(FIXTURE_MODE)) {
  throw new Error(`unsupported ACCOUNT_RUNTIME_THINKING_WIRE_FIXTURE_MODE: ${FIXTURE_MODE}`)
}
...
const ROUNDS = IS_FIXTURE
  ? 5
  : Number.parseInt(process.env.ACCOUNT_RUNTIME_THINKING_WIRE_ROUNDS || '5', 10)
...
const EXPECTED_CLAUDE_VERSION = optionalVersionEnvironment('ACCOUNT_RUNTIME_EXPECTED_CLAUDE_VERSION')
const MINIMUM_CLAUDE_VERSION = versionEnvironment('ACCOUNT_RUNTIME_MIN_CLAUDE_VERSION', '2.1.197')
...
const REDIS_PREFIXES = Object.fromEntries(ENDPOINTS.map((endpoint) => [
  endpoint,
  `account_runtime:validation:${RUN_ID}:${endpoint}`,
]))
const REDIS_FOREIGN_SENTINEL = `account_runtime:validation-foreign:${RUN_ID}`
...
const MAX_WALL_DURATION_MS = parseOptionalIntegerEnvironment(
  'ACCOUNT_RUNTIME_THINKING_WIRE_MAX_WALL_MS',
  20 * 60 * 1000,
  60_000,
  60 * 60 * 1000,
)
...
const SOURCE_MANIFEST_EXCLUDES = [
  ':(exclude,icase,glob)**/account_runtime_idc_users*.txt',
]
...
let REDIS_URL = IS_FIXTURE
  ? ''
  : requiredEnvironment('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL')

if (ROUNDS !== 5) {
  throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_ROUNDS must be exactly 5 for the A09/D07 wire gate')
}

const DATABASE_OWNER = IS_FIXTURE
  ? 'fixture_owner'
  : requiredEnvironment('ACCOUNT_RUNTIME_THINKING_WIRE_DATABASE_OWNER')
if (!/^[a-z0-9_]{8,36}$/.test(DATABASE_OWNER)) {
  throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_DATABASE_OWNER must match ^[a-z0-9_]{8,36}$')
}

const POSTGRES_URLS = IS_FIXTURE
  ? { cli: 'postgres://signal-fixture.invalid/cli', ide: 'postgres://signal-fixture.invalid/ide' }
  : {
      cli: requiredEnvironment('ACCOUNT_RUNTIME_THINKING_WIRE_CLI_POSTGRES_URL'),
      ide: requiredEnvironment('ACCOUNT_RUNTIME_THINKING_WIRE_IDE_POSTGRES_URL'),
    }
...
const CLAUDE = IS_FIXTURE
  ? ''
  : resolveExecutable(process.env.ACCOUNT_RUNTIME_CLAUDE_BINARY || 'claude')
const PSQL = IS_FIXTURE
  ? ''
  : resolveExecutable(process.env.ACCOUNT_RUNTIME_PSQL_BINARY || 'psql')
...
function postgresOwnerMarker(owner, endpoint) {
  return `account-runtime-thinking-wire-owner:${owner}:${endpoint}`
}
...
function validateIsolationUrls(postgresUrls, redisUrl, owner) {
  const parsedPostgres = Object.entries(postgresUrls).map(([endpoint, value]) => {
    ...
    try {
      ...
      throw new Error(`ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL must be a valid URL`)
    }
    if (!['postgres:', 'postgresql:'].includes(parsed.protocol)) {
      throw new Error(`ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL must use PostgreSQL`)
    }
    if (!isLoopbackHost(parsed.hostname)) {
      throw new Error(`ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL must target loopback`)
    }
    if (parsed.hash) {
      throw new Error(`ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL must not contain a fragment`)
    }
    for (const name of parsed.searchParams.keys()) {
      if (name !== 'sslmode') {
        throw new Error(
          `ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL contains unsupported query parameter ${name}`,
        )
      }
    }
    const database = decodeURIComponent(parsed.pathname.replace(/^\//, ''))
    const expectedDatabase = `account_runtime_thinking_wire_${owner}_${endpoint}`
    if (database !== expectedDatabase) {
      throw new Error(
        `ACCOUNT_RUNTIME_THINKING_WIRE_${endpoint.toUpperCase()}_POSTGRES_URL must name ${expectedDatabase}`,
      )
    }
    return {
      endpoint,
  ...
  try {
    ...
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL must be a valid URL')
  }
  if (parsedRedis.protocol !== 'redis:') {
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL must use plain Redis on isolated loopback')
  }
  if (!isLoopbackHost(parsedRedis.hostname)) {
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL must target an isolated loopback Redis')
  }
  if (parsedRedis.username && !parsedRedis.password) {
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL username requires a password')
  }
  if (parsedRedis.search || parsedRedis.hash) {
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL must not contain query parameters or a fragment')
  }
  const redisDatabase = parsedRedis.pathname.replace(/^\//, '') || '0'
  if (!/^\d+$/.test(redisDatabase)) {
    throw new Error('ACCOUNT_RUNTIME_THINKING_WIRE_REDIS_URL must contain a numeric database')
  }
...
async function inspectPostgresIdentity(endpoint, requireEmpty) {
  ...
  const output = await commandOutput(PSQL, [
    ...
    env: postgresCommandEnvironment(
      ...
      `account_runtime_thinking_wire_${DATABASE_OWNER}_${endpoint}`,
    ),
  })
...
async function waitForHealth(baseUrl, child, timeoutMs = 60_000) {
  ...
  while (Date.now() < deadline) {
    throwIfShutdownRequested()
    if (child.spawnError) {
      throw new Error('failed to start isolated account-runtime service')
    }
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`account-runtime service exited before health check for isolated ${baseUrl}`)
    }
    try {
...
function selectedWireFacts(bodyBuffer, request, url, detectedEndpoint) {
  ...
  return {
    ...
    path: url.pathname,
    target: String(request.headers['x-account-runtime-target'] || ''),
...
function exactProtocolViolations({ endpoint, kind, request, url, body }) {
  ...
  const expectedHost = endpoint === 'cli'
    ? (kind === 'model_discovery' ? 'management.us-east-1.account-runtime.local' : 'runtime.us-east-1.account-runtime.local')
    : 'q.us-east-1.account-runtime.local'
  ...
  if (kind === 'model_discovery' && endpoint === 'cli') {
    ...
    if (target !== 'AccountRuntimeService.ListAvailableModels') violations.push('target')
    if (contentType !== 'application/json') violations.push('content_type')
    if (String(request.headers.accept || '') !== '*/*') violations.push('accept')
    if (url.searchParams.get('origin') !== 'ACCOUNT_RUNTIME_CLI') violations.push('query_origin')
    if (body?.origin !== 'ACCOUNT_RUNTIME_CLI') violations.push('body_origin')
    ...
    if (url.pathname !== '/thinking-wire/') violations.push('path')
    if (target !== 'AccountRuntimeStreamingService.GenerateAssistantResponse') {
      violations.push('target')
    }
    if (contentType !== 'application/json') violations.push('content_type')
    const origin = body?.conversationState?.currentMessage?.userInputMessage?.origin
    if (origin !== 'ACCOUNT_RUNTIME_CLI') violations.push('origin')
  } else {
...
function createFakeAccountRuntimeUpstream() {
  const records = []
  const server = http.createServer((request, response) => {
    void (async () => {
      ...
      if (url.pathname.endsWith('/getUsageLimits')) {
        ...
        if (String(request.headers.host || '') !== 'q.us-east-1.account-runtime.local') {
  ...
  const tracked = registerServer('fake-account-runtime', server)
  return {
    records,
    tracked,
    async listen(port) {
...
function isolatedConfig({ endpoint, servicePort, upstreamPort }) {
  return {
    ...
    upstreamBaseUrl: `http://127.0.0.1:${upstreamPort}/thinking-wire`,
...
function isolatedServiceEnvironment(endpoint, servicePort) {
  return minimalEnvironment({
    ...
    ACCOUNT_RUNTIME_POSTGRES_URL: POSTGRES_URLS[endpoint],
...
function validateCase({ endpoint, effort, cli, inboundRecords, wireRecords }) {
  ...
  if (wireRecords.length !== 1) violations.push('unexpected_account_runtime_inference_count')
  ...
  if (wireEffort !== expectedEffort) violations.push('account_runtime_wire_effort_mismatch')
  if (!FAKE_MODEL_IDS.includes(wire?.model)) violations.push('account_runtime_wire_model_not_advertised')
  ...
  if ((wire?.protocolViolations || []).length > 0) violations.push('account_runtime_wire_protocol_mismatch')

  const wireThinking = wire?.additionalModelRequestFields?.thinking ?? null
  if (endpoint === 'cli') {
    if (wire?.origin !== 'ACCOUNT_RUNTIME_CLI') violations.push('cli_origin_mismatch')
  } else {
...
async function main() {
  ...
  const fake = createFakeAccountRuntimeUpstream()
  const endpointRuns = []
  let report = null
    ...
    report = {
      ...
      isolation: {
        ...
        serviceEnvironmentOverridesPinned: ENDPOINTS.every((endpoint) => {
          ...
          return environment.ACCOUNT_RUNTIME_POSTGRES_URL === POSTGRES_URLS[endpoint]
            && environment.ACCOUNT_RUNTIME_REDIS_URL === REDIS_URL
            && environment.ACCOUNT_RUNTIME_POSTGRES_MIGRATE_ON_START === 'true'
            && environment.ACCOUNT_RUNTIME_POSTGRES_COMPRESS_USAGE_ROLLUPS_ON_START === 'false'
        }),
...
function contractWireFacts(endpoint, thinking) {
  return {
    ...
    origin: endpoint === 'cli' ? 'ACCOUNT_RUNTIME_CLI' : 'AI_EDITOR',
...
function runContractFixture() {
  ...
  const isolation = validateIsolationUrls({
    cli: `postgres://127.0.0.1/account_runtime_thinking_wire_${contractOwner}_cli`,
    ide: `postgres://127.0.0.1/account_runtime_thinking_wire_${contractOwner}_ide`,
  ...
  const protocolCases = [
    {
      ...
      kind: 'model_discovery',
      method: 'POST',
      url: 'http://127.0.0.1/thinking-wire/?origin=ACCOUNT_RUNTIME_CLI',
      headers: {
        host: 'management.us-east-1.account-runtime.local',
        tokentype: 'API_KEY',
        accept: '*/*',
        'content-type': 'application/json',
        'x-account-runtime-target': 'AccountRuntimeService.ListAvailableModels',
      },
      body: { origin: 'ACCOUNT_RUNTIME_CLI' },
    },
    {
      endpoint: 'ide',
      kind: 'model_discovery',
      ...
      headers: {
        host: 'q.us-east-1.account-runtime.local',
        tokentype: 'API_KEY',
    ...
    },
    {
      ...
      headers: {
        host: 'runtime.us-east-1.account-runtime.local',
        tokentype: 'API_KEY',
        'content-type': 'application/json',
        'x-account-runtime-target': 'AccountRuntimeStreamingService.GenerateAssistantResponse',
      },
      body: { conversationState: { currentMessage: { userInputMessage: { origin: 'ACCOUNT_RUNTIME_CLI' } } } },
    },
    {
      endpoint: 'ide',
      kind: 'inference',
  ...
  const forbiddenInheritedNames = [
    ...
    'LOCAL_UPSTREAM_API_KEY',
    'ACCOUNT_RUNTIME_POSTGRES_URL',
    'ACCOUNT_RUNTIME_REDIS_URL',
    'DATABASE_URL',
    'REDIS_URL',
    'PGPASSWORD',
    'HTTP_PROXY',
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    environment: {
      ...
      postgresPinned: serviceEnvironment.ACCOUNT_RUNTIME_POSTGRES_URL === POSTGRES_URLS.cli,
      redisPinned: serviceEnvironment.ACCOUNT_RUNTIME_REDIS_URL === REDIS_URL,
      migratePinned: serviceEnvironment.ACCOUNT_RUNTIME_POSTGRES_MIGRATE_ON_START === 'true',
      compressionPinned:
        serviceEnvironment.ACCOUNT_RUNTIME_POSTGRES_COMPRESS_USAGE_ROLLUPS_ON_START === 'false',
...
if (FIXTURE_MODE === 'contract') {
  ...
  main().catch(async (error) => {
    ...
    process.stderr.write(`thinking effort account runtime wire validation failed: ${error.message}\n`)
    process.exitCode = 1
  })
}
