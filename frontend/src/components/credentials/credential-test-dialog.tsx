...
import { useCredentialTest } from '@/hooks/use-credentials'
...
function defaultModel(models: ModelPrice[]): string {
...
export function CredentialTestDialog({
  ...
  const modelList = useMemo(() => (open ? pricing.data ?? [] : []), [open, pricing.data])
  ...
  useEffect(() => {
    if (!open || model || modelList.length === 0) return
    setModel(defaultModel(modelList))
  }, [model, modelList, open])
        ...
        <div className="space-y-4">
          <div className="rounded-md border bg-muted/30 p-3 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {credential.endpoint && (
                <Badge variant="secondary">{credential.endpoint}</Badge>
              )}
              {credential.disabled && <Badge variant="destructive">已停用</Badge>}
            </div>
            <div className="mt-2 text-xs text-muted-foreground">
              成功 {credential.successCount.toLocaleString('zh-CN')} · 失败{' '}
          ...
          <div className="space-y-2">
            <Label htmlFor={`credential-test-model-${credential.id}`}>模型</Label>
            <Select
              value={model}
              onValueChange={setModel}
              disabled={pricing.isLoading || testCredential.isPending}
            >
              <SelectTrigger id={`credential-test-model-${credential.id}`}>
