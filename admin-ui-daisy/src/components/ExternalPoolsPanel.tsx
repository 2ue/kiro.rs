      ...
      <SectionCard title="外部账号策略" actions={<Button size="sm" color="primary" loading={savingConfig} onClick={saveConfig}><Save className="h-4 w-4" />保存策略</Button>}>
        <div className="space-y-5">
          ...
          <PolicyBlock
            ...
            description="只影响选择“按当前请求路径整理 usage”的外部账号。本地账号和“透传上游 usage”的外部账号不会受影响。"
          >
            <div className="space-y-4">
              <HintBox>
                生效条件：请求进入外部账号，并且该外部账号的 Usage 上报口径为“按当前请求路径整理 usage”。如果外部账号选择“透传上游 usage”，下面配置不会改动 usage。
              </HintBox>
              <div className="grid gap-4 lg:grid-cols-2">
                <FormSection title="缓存读写补偿" description="按当前请求路径整理 usage 后，对缓存读写用量做补偿。">
                  <div className="grid gap-3 sm:grid-cols-2">
            ...
            <SectionCard key={pool.id} title={`#${pool.id} ${pool.name}`}>
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-base-content/50">{poolUsageSummary(pool, configDraft)} · {poolBodyModeSummary(pool)} · 认证：{authLabel(pool.authType)} · 模型：{poolModelMappingSummary(pool)} · {supportedModelsSummary(pool.supportedModels)}{runtime?.cooldownRemainingSecs ? ` · 冷却 ${runtime.cooldownRemainingSecs}s` : ''}</div>
          ...
          <FormSection title="Usage 上报口径" description="只决定当前外部账号返回给下游的 usage 是透传上游，还是按当前请求路径整理；非 usage 内容不受影响。">
            <div className="space-y-3">
              <SelectBox label="下游 usage" value={draft.usageProjectionMode} disabled={saving} onChange={(usageProjectionMode) => onDraftChange((prev) => ({ ...prev, usageProjectionMode: usageProjectionMode as ExternalPoolFormDraft['usageProjectionMode'] }))}>
                <Select.Option value="pass_through">透传上游 usage</Select.Option>
                <Select.Option value="current_path_policy">按当前请求路径整理 usage</Select.Option>
              </SelectBox>
        ...
        <FormSection title="调度资格" description="只决定该外部账号是否允许承接某些模型；不改变请求体里的 model，也不影响模型映射规则。">
          <div className="grid gap-3 md:grid-cols-[1fr_220px]">
            ...
            <div className="space-y-2">
              ...
              >
                <Select.Option value="">选择账号</Select.Option>
                {credentialOptions.map((credential) => (
                  <Select.Option key={credential.id} value={String(credential.id)}>
                    #{credential.id} {credential.email || credential.maskedApiKey || credential.authMethod || '账号'}
                  </Select.Option>
                ))}
              </SelectBox>
              <Button type="button" size="sm" color="ghost" className="w-full" loading={syncingModels} disabled={saving || syncingModels || !isEdit || !syncCredentialId || !onSyncSupportedModels} onClick={syncSupportedModels}>
                同步支持模型
              </Button>
              {!isEdit && <div className="text-xs text-base-content/60">创建后编辑外部账号可从本地账号同步。</div>}
            </div>
          </div>
        </FormSection>
        ...
        <FormSection title="请求体处理" description="控制发往该外部账号前是否进入本系统 body 处理链路。">
          <div className="grid gap-3">
            <div className="space-y-3">
                ...
                >
                <Select.Option value="normalized">标准处理</Select.Option>
                <Select.Option value="raw_passthrough">Raw 透传</Select.Option>
              </SelectBox>
...
function requestBodyModeDescription(mode: ExternalPool['requestBodyMode'] | undefined) {
  if (mode === 'raw_passthrough') {
    return '请求体不进入消息解析、图片处理、schema 修正和 payload guard。下游 usage 是透传上游还是按路径整理，由当前外部账号的 Usage 上报口径决定；是否改写顶层 model 由下方模型处理配置单独控制。'
  }
