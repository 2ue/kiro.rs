...
const baseUrl = process.env.ADMIN_UI_URL || 'http://127.0.0.1:9024/console'
...
async function main() {
  ...
  await fillFileInput(page, '从文件填充', 'credential.json', JSON.stringify({ auth_method: 'api_key', kiro_api_key: 'ksk_file_fake', email: 'file-smoke@example.com' }))
  ...
  await fillFileInput(page, '选择文件', 'credentials.jsonl', '{"auth_method":"api_key","kiro_api_key":"ksk_batch_fake_1","email":"batch1@example.com"}\n{"authMethod":"api_key","kiroApiKey":"ksk_batch_fake_2","email":"batch2@example.com"}')
  ...
  await fillFileInput(page, '选择文件', 'kam.json', JSON.stringify({ accounts: [{ email: 'kam@example.com', status: 'ok', machine_id: 'machine-smoke', credentials: { refresh_token: 'rt_fake' } }] }))
  ...
  if (await page.getByText('同步价格').count()) throw new Error('使用记录页面不应出现模型价格同步入口')
  await page.getByPlaceholder('搜索模型、账号、会话、错误').fill('claude')
  await page.getByPlaceholder('模型', { exact: true }).fill('claude-sonnet')
  ...
  await page.getByRole('tab', { name: /高缓存/ }).click()
  await expectText(page, '高缓存模拟')
  await page.getByRole('tab', { name: /路径上报/ }).click()
  ...
  await page.getByRole('tab', { name: /兼容诊断/ }).click()
  await expectText(page, '兼容与诊断')
  await page.getByRole('tab', { name: /路径上报/ }).click()
  logs.push('运行配置页面、配置分组 tab 和新增路径覆盖交互通过（未保存）')
  ...
  await page.getByTitle('切换主题').first().click()
