...
# Account Runtime Prod Evidence Audit
...
2. Read `references/account-runtime-evidence-sources.md` and `references/evidence-map.md` before the first production SSH command.
3. Establish SSH access. If direct password login was requested or credentials were provided inline, use direct password SSH immediately; prefer reusing a short-lived control socket for the rest of the run. Record the command purpose in `commands.md`, but do not record the password.
...
- component: app, gateway, PostgreSQL, Redis, Docker, host OS, external pool, upstream Account Runtime/Anthropic, usage accounting.
