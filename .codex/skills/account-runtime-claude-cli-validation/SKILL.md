...
# Account Runtime Claude CLI Validation
...
- Before touching an existing local service, identify the exact listener PID/command and confirm it is this project’s `account-runtime`; stop only that process.
...
- If later runtime tests need a binary, copy only the finished binary out of `$CARGO_TARGET_DIR` before the wrapper exits, record its SHA-256, and pass its absolute immutable path as `ACCOUNT_RUNTIME_BINARY`. A runner must not discover `target/debug`, `target/release`, or silently invoke Cargo.
...
ACCOUNT_RUNTIME_FROZEN_BINARY="$candidate_root/account-runtime" \
  ...
  bash -lc 'cargo fmt --check && cargo test && cargo build --release && install -m 755 "$CARGO_TARGET_DIR/release/account-runtime" "$ACCOUNT_RUNTIME_FROZEN_BINARY"'
shasum -a 256 "$candidate_root/account-runtime"
export ACCOUNT_RUNTIME_BINARY="$candidate_root/account-runtime"
```
...
if [ -n "$pid" ]; then
  ...
  case "$cmd" in *account-runtime*) kill "$pid" ;; *) echo "9022 is not account-runtime: $cmd" >&2; exit 1 ;; esac
fi
"$ACCOUNT_RUNTIME_BINARY" -c config.json --credentials credentials.json
```
