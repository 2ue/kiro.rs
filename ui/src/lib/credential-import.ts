...
function extractCredentialItems(value: unknown): unknown[] {
  if (Array.isArray(value)) return value.flatMap(extractCredentialItems)
  if (!isObject(value)) return []
  if (Array.isArray(value.credentials)) return value.credentials.flatMap(extractCredentialItems)
  if (Array.isArray(value.accounts)) return value.accounts.flatMap(extractCredentialItems)
