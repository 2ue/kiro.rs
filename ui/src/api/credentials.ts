...
function normalizeRuntimeConfig(data: RuntimeConfigWire): RuntimeConfig {
  const { accountRuntime: accountRuntimeValue, externalPools, ...current } = data
  const accountRuntime = accountRuntimeValue ?? externalPools ?? defaultAccountRuntimeConfig()
  return {
    ...current,
...
export async function setCredentialRateLimitAutoDisable(id: number, req: SetCredentialRateLimitAutoDisableRequest): Promise<SuccessResponse> {
