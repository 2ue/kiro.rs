...
import { PageContainer, PageHeader, SectionCard, Field, useConfirm, ErrorState, LoadingState } from '@/components/patterns'
import {
  Button,
  Input,
  Textarea,
...
function ToggleField({
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      <Switch checked={checked} disabled={disabled} onCheckedChange={onChange} className="shrink-0 mt-0.5" />
    </div>
  )
}
...
function ImpactGroupHeader({
  ...
  return (
    <div className={cn('col-span-full rounded-lg border border-border bg-muted/40 px-3 py-2', muted && 'opacity-50')}>
      <div className="mb-1 flex flex-wrap items-center gap-2">
        <span className="rounded bg-primary/10 px-1.5 py-0.5 text-[0.65rem] font-semibold text-primary">{label}</span>
...
function ModeSelect({
  ...
  return (
    <Select value={value} onValueChange={(v) => onChange(v as ReportedUsageFieldMode)} disabled={disabled}>
      <SelectTrigger size="sm">
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="raw">显示原始值</SelectItem>
        <SelectItem value="preserve">保留当前计算结果</SelectItem>
        <SelectItem value="sample-max">按上限自动调整</SelectItem>
        <SelectItem value="sample-target">按目标自动调整</SelectItem>
      </SelectContent>
    </Select>
  )
}
    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          onChange={(e) => onChange(numberValue(e.target.value, min ?? 0))}
        />
        <span className="min-w-[4rem] shrink-0 text-xs text-muted-foreground">{suffix}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-lg border border-border p-3 space-y-2.5">
      ...
      <div className="space-y-2.5">
        ...
        <div className="text-xs text-muted-foreground italic">{reportedUsageModeDescription(value.mode)}</div>
        {fieldNeedsMax(value) && (
          <PolicyNumberInput
            title="展示上限"
...
function AccessKeysPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title="请求调用 Key"
        description="给客户端调用模型接口时使用。可以按客户端分配不同 Key，新增、编辑、删除后立即生效。"
        actions={
          <Button size="sm" disabled={loading || creating} onClick={generateRequestKey}>
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            ...
            onChange={(e) => setManualRequestApiKey(e.target.value)}
          />
          <Button
            variant="outline"
            size="sm"
            className="shrink-0"
            disabled={loading || creating}
            onClick={() => setManualRequestApiKey(generateLocalRequestApiKey())}
          >
          ...
          <Button
            size="sm"
            className="shrink-0"
            disabled={loading || creating || !manualRequestApiKey.trim()}
            onClick={addManualRequestKey}
          >
        ...
        <div className="mt-3 divide-y divide-border rounded-lg border border-border">
          ...
          {!loading && requestKeys.length === 0 && (
            <div className="px-4 py-3">
              <ErrorState message="未配置请求 Key，请先生成或手动新增一个。" />
            </div>
          )}
          {!loading &&
            requestKeys.map((item) => {
                ...
                <div key={item.id} className="px-4 py-3">
                  <div className="mb-2 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() => toggleRequestKeyVisible(item.id)}
                        title={visible ? '隐藏请求 Key' : '显示完整请求 Key'}
                      >
                      ...
                      <Button
                        ...
                        size="xs"
                        disabled={busy || editing}
                        onClick={() => copy('请求 Key', item.apiKey)}
                      >
                      ...
                      {!editing && (
                        <Button
                          variant="ghost"
                          size="xs"
                          disabled={busy || Boolean(editingRequestKeyId)}
                          onClick={() => startEditRequestKey(item)}
                        >
                  ...
                  <div className="space-y-2">
                    <Input
                      ...
                      disabled={busy}
                      onChange={(e) => setRequestKeyDraft(e.target.value)}
                    />
                    {editing && (
                      <div className="flex flex-wrap justify-end gap-2">
                        <Button
                          ...
                          size="sm"
                          disabled={busy}
                          onClick={() => setRequestKeyDraft(generateLocalRequestApiKey())}
                        >
                          <Wand2 className="h-4 w-4" />
                          随机生成
                        </Button>
                        <Button
                          size="sm"
                          disabled={busy || !requestKeyDraft.trim()}
                          onClick={() => saveEditedRequestKey(item)}
                        >
                          {busy ? <Spinner size="sm" /> : <Save className="h-4 w-4" />}
                          保存
                        </Button>
                        <Button variant="outline" size="sm" disabled={busy} onClick={cancelEditRequestKey}>
                          <X className="h-4 w-4" />
                          取消
                        </Button>
                      </div>
      ...
      <SectionCard
        title="后台登录 Key"
        description="这是登录页输入的密码，也用于管理后台的后续操作。修改成功后，当前浏览器会自动切换到新 Key。"
      >
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            readOnly
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              disabled={saving}
              onChange={(e) => setNextAdminApiKey(e.target.value)}
            />
            <Button
              size="sm"
              className="shrink-0"
              disabled={saving || !nextAdminApiKey.trim()}
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {onDelete && (
            <Button variant="outline" size="xs" onClick={onDelete} title="删除这条入口规则">
              <Trash2 className="h-3.5 w-3.5" />
              删除规则
            </Button>
          )}
      ...
      {!value.enabled && (
        <div className="rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-xs leading-5 text-warning-foreground">
        ...
        <>
          ...
          <div className="mt-2 grid gap-3 xl:grid-cols-3">
            <PolicyNumberInput
              ...
              min={0}
              suffix="Token"
              onChange={(finalCacheReadMaxTokens) => onChange({ ...value, finalCacheReadMaxTokens })}
            />
            <PolicyNumberInput
              title="最终上限扣减下限"
              ...
              min={0}
              suffix="Token"
              onChange={(finalCacheReadJitterMinTokens) => onChange({ ...value, finalCacheReadJitterMinTokens })}
            />
            <PolicyNumberInput
              title="最终上限扣减上限"
              ...
              min={0}
              suffix="Token"
              onChange={(finalCacheReadJitterMaxTokens) => onChange({ ...value, finalCacheReadJitterMaxTokens })}
            />
          </div>
        </>
      )}
...
export function ConfigPage() {
  ...
  const save = () => {
    ...
    if (
      [
        next.credentialRateLimitCooldownSecs,
        next.credentialServerErrorCooldownSecs,
        next.credentialNetworkErrorCooldownSecs,
        next.credentialStreamErrorCooldownSecs,
        next.credentialProtocolErrorCooldownSecs,
        next.credentialAuthErrorCooldownSecs,
    ...
    if (next.payloadGuardEnabled && next.payloadGuardMaxBytes > 0 && next.payloadGuardMaxBytes < 65536)
      return toast.error('请求大小处理阈值必须为 0 或不小于 65536 字节')
    if (
      next.payloadGuardEnabled &&
      next.payloadGuardMaxBytes > 0 &&
      next.payloadGuardMaxBytes - next.payloadGuardSafetyMarginBytes < 65536
    )
      return toast.error('安全余量不能过大，实际处理目标不能小于 65536 字节')
  ...
  const selectedUsageKey =
    selectedUsagePath === '__default' || draft.reportedUsage.pathOverrides[selectedUsagePath]
      ? selectedUsagePath
      : '__default'
  const showRuntimeSave = activeTab !== 'access'

  if (config.isLoading) return <LoadingState text="加载配置中..." />
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        subtitle={pageMeta.config.subtitle}
      />

      <div className="flex gap-6">
        {/* 左侧导航 */}
        <aside className="hidden w-48 shrink-0 md:block" aria-label="配置分类">
          <div className="flex flex-col gap-1" role="tablist" aria-label="配置分类">
            {configTabs.map((tab) => (
              <button
                key={tab.key}
                type="button"
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block truncate text-[0.68rem]">{tab.description}</span>
                </span>
              </button>
            ))}
          </div>
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mb-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2.5">
              <span className="text-muted-foreground">{activeTabMeta.icon}</span>
              <div>
                <h3 className="text-base font-semibold">{activeTabMeta.label}</h3>
                <p className="text-xs leading-5 text-muted-foreground">{activeTabMeta.description}</p>
              </div>
            </div>
            {showRuntimeSave && (
          ...
          <div className="space-y-4">
            ...
            {activeTab === 'limits' && (
              <SectionCard title="请求容量" description="控制每个账号和全局的承载量，以及请求等待、重试和超时。">
                <SectionGrid>
            ...
            {activeTab === 'cooldown' && (
              <SectionCard title="错误恢复" description="设置账号遇到不同错误后暂停多久，以及连续失败时如何延长暂停时间。">
                <SectionGrid>
            ...
            {activeTab === 'scheduler' && (
              <SectionCard title="账号选择权重" description="决定系统优先使用哪些账号，让请求尽量分散在更稳定的账号上。">
                <SectionGrid>
            ...
            {activeTab === 'warmup' && (
              <SectionCard title="新账号预热" description="让新账号先少量参与服务，确认稳定后再逐步恢复正常使用。">
                <SectionGrid>
            ...
            {activeTab === 'payload' && (
              <SectionCard title="请求大小保护" description="设置请求过大时的处理方式、触发阈值和基础压缩开关。">
                <ConfigGroup>
                  <SectionGrid>
                    <ImpactGroupHeader label="全局影响" title="每次请求发送前都会检查" description="开启后，系统会在发送前做必要的压缩、格式修正和大小检查。" />
                    ...
                    <Field label="过大请求处理方式" description={'可选择发送前先裁剪，也可以在收到“内容过长”错误后再裁剪并重试。'}>
                      <Select value={payloadGuardMode} disabled={!draft.payloadGuardEnabled} onValueChange={(v) => setDraft((prev) => ({ ...prev, payloadGuardMode: v as PayloadGuardMode }))}>
                    ...
                    <ImpactGroupHeader label="条件阈值" title="控制什么时候开始处理过大请求" description="这里设置的是本地处理阈值，不是模型上下文上限。填 0 表示关闭按大小触发的处理。" />
                    ...
                    <ImpactGroupHeader label="条件分支" title={payloadConditionTitle} description={payloadConditionDescription} muted={!payloadSizeLimitEnabled} />
                  </SectionGrid>
                </ConfigGroup>
              </SectionCard>
            )}
            ...
            {activeTab === 'payloadHistory' && (
              <SectionCard title="旧内容清理" description="当请求超过阈值时，先处理旧对话、历史工具结果和网页抓取内容。">
                <ConfigGroup>
                  <SectionGrid>
                    <ImpactGroupHeader label="生效条件" title={payloadConditionTitle} description={payloadConditionDescription} muted={!payloadSizeLimitEnabled} />
            ...
            {activeTab === 'payloadFallback' && (
              <SectionCard title="当前内容兜底" description="旧内容清理后仍然过大时，再处理当前消息、当前文档和图片。">
                <ConfigGroup>
                  <SectionGrid>
                    <ImpactGroupHeader
                      label="兜底分支"
            ...
            {activeTab === 'cacheRead' && (
              <SectionCard title="缓存命中展示" description="控制页面和响应里展示的缓存用量，让统计更接近预期展示口径。">
                <SectionGrid>
            ...
            {activeTab === 'cacheCreate' && (
              <SectionCard title="缓存创建频次" description="控制缓存创建数值出现的频率，避免展示结果过于频繁或过大。">
                <SectionGrid>
                  ...
                  <Field label="控制维度" description="选择按账号分别控制，还是按同一个会话和模型统一控制。">
                    <Select
                      value={draft.promptCacheCreationControl.scopeMode}
                      disabled={!draft.promptCacheCreationControl.enabled}
                      onValueChange={(v) => setDraft((prev) => ({ ...prev, promptCacheCreationControl: { ...prev.promptCacheCreationControl, scopeMode: v as 'credential_conversation_model' | 'conversation_model' } }))}
                    >
                      <SelectTrigger size="sm"><SelectValue /></SelectTrigger>
                      <SelectContent>
            ...
            {activeTab === 'usage' && (
              <SectionCard title="用量展示规则" description="按不同入口设置看到的输入、输出和缓存用量。只影响展示口径，不改变实际请求内容。">
                <div className="flex gap-4">
                  <div className="flex w-44 shrink-0 flex-col gap-1" role="tablist" aria-label="用量展示规则入口">
                    <button
                      type="button"
                      role="tab"
                    ...
                    >
                      ...
                      <span className="text-xs">未匹配入口时使用</span>
                    </button>
                    {reportedUsagePaths.map((prefix) => (
                      ...
                      >
                        ...
                        <span className="text-xs">{draft.reportedUsage.pathOverrides[prefix]?.enabled ? '已启用' : '已关闭'}</span>
                      </button>
                    ))}
                    <Button
                      type="button"
                  ...
                  <div className="min-w-0 flex-1">
                    {selectedUsageKey === '__default' ? (
                      ...
                      <div className="space-y-3">
                        <Field label="入口前缀" description="只影响匹配这个前缀的入口。不同前缀互相独立。">
                          <Input
            ...
            {activeTab === 'compat' && (
              <SectionCard title="兼容与模型" description="选择接口兼容方式，并维护模型名称映射。">
                <div className="space-y-4">
                  <Field label="兼容模式" description="日常使用建议保持兼容模式；严格模式会尽量减少代理侧处理；调试模式会输出更多排查信息。">
                    <Select value={draft.compatProfile} onValueChange={(v) => setDraft((prev) => ({ ...prev, compatProfile: v as CompatProfile }))}>
                      <SelectTrigger size="sm"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <Field label="Kiro 工作模式" description="控制 Kiro 侧使用的工作模式。一般保持默认即可，只有需要特定模式时再调整。">
                    <Select value={draft.kiroAgentModeStrategy} onValueChange={(v) => setDraft((prev) => ({ ...prev, kiroAgentModeStrategy: v as KiroAgentModeStrategy }))}>
                      <SelectTrigger size="sm"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="vibe">vibe（默认兼容）</SelectItem>
                        <SelectItem value="spec">spec（强制规格模式）</SelectItem>
                        <SelectItem value="auto">auto（按账号协议自动）</SelectItem>
                      </SelectContent>
                    </Select>
                  </Field>
                  <Field label="模型解析策略" description="控制模型名称如何匹配。越严格越少自动转换，越兼容越适合常见简称。">
                    <Select value={draft.modelResolutionMode} onValueChange={(v) => setDraft((prev) => ({ ...prev, modelResolutionMode: v as ModelResolutionMode }))}>
                      <SelectTrigger size="sm"><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <Field label="模型映射规则" description="把常见模型简称或别名转换成实际可用的模型名。">
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={modelCapabilities.isLoading}
                          onClick={() => {
                      ...
                      <div className="text-xs text-muted-foreground">
                        当前规则 {draft.modelMapping.rules.length} 条；可生成默认规则 {defaultModelMappingRules.length} 条。
                      </div>
                    </div>
                  </Field>
                  <SectionGrid>
            ...
            {activeTab === 'stats' && (
              <SectionCard title="后台统计" description="控制页面统计的判断标准，不改变实际请求。">
                <SectionGrid>
