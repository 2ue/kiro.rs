...
import { Badge, Button, Input, Textarea, Switch, Spinner } from '@/components/ui'
...
export function ProxyEditorModal({ open, resource, onClose }: {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (saved.id) toast.success(isEditing ? '代理资源已更新' : '代理资源已创建')
      onClose()
    } catch (error) {
  ...
  const pending = create.isPending || update.isPending || testProxy.isPending
