      ...
      {pool && (
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-muted/40 p-4">
            ...
            <div className="mt-2 break-all text-sm text-muted-foreground">{pool.baseUrl}</div>
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted/40 p-4 font-mono text-sm">
            <div className="space-y-1">
              ...
              <div><span className="text-muted-foreground">发送测试消息：</span><span> "{prompt.trim() || DEFAULT_TEST_PROMPT}"</span></div>
            </div>
            <div className="mt-4 border-t border-border pt-4">
              ...
              {result && (
                <div className={result.ok ? 'space-y-3 text-success' : 'space-y-3 text-destructive'}>
                  <div>
          ...
          <div className="flex flex-wrap justify-between gap-3 text-sm text-muted-foreground">
            <span>测试模型：{modelOptions.find((o) => o.id === model)?.label || model}</span>
