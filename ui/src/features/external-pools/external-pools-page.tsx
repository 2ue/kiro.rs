            ...
            <div className="space-y-4">
              <FormSection title="容量与排队" description={waitModeActive ? '外部账号满并发时会等待容量。' : '外部账号满并发时不会排队。'}>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <NumberBox disabled={!externalEnabled} label="连续失败计数（兼容）" description="保留旧配置字段；普通瞬态失败只降低调度优先级，不会触发池级冷却。池级冷却仅用于模型不可用、端点错误或自动禁用等极端状态。" suffix="次" min={0} value={configDraft.externalPoolTransientFailureCooldownThreshold} onChange={(v) => setConfigDraft((p) => ({ ...p, externalPoolTransientFailureCooldownThreshold: v }))} />
