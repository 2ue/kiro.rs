...
async fn execute_request(
    ...
    let request_id = header_string(&headers, "request-id")
        ...
        .or_else(|| header_string(&headers, "x-runtime-requestid"));
    ...
    match scenario {
        ...
        Scenario::JsonException200 => (
            ...
            [
                ...
                ("x-runtime-requestid", request_id.as_str()),
            ],
            Json(json!({
...
fn wants_local_upstream_eventstream(
    ...
    if headers
        .get("x-account-runtime-fixture")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|fixture| fixture.eq_ignore_ascii_case("eventstream"))
    {
        return true;
    }
    false
...
fn is_local_upstream_model_discovery_request(uri: &Uri, headers: &HeaderMap) -> bool {
    ...
    if headers
        .get("x-account-runtime-fixture")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|fixture| fixture.eq_ignore_ascii_case("models"))
    {
        return true;
    }
    false
...
async fn capture_fake_request(
    ...
    for (name, value) in headers.iter() {
        ...
        let value = if matches!(
            key.as_str(),
            "authorization" | "x-api-key" | "x-runtime-security-token"
        ) {
            "<redacted>".to_string()
        } else {
...
fn fake_upstream_model_discovery_message(request_id: &str) -> Response {
    (
        ...
        [
            ("request-id", request_id),
            ("x-runtime-requestid", request_id),
        ],
        Json(json!({
            "models": [
...
fn local_upstream_eventstream_response(
    ...
    Response::builder()
        ...
        .header(header::CACHE_CONTROL, "no-cache")
        .header("request-id", &request_id)
        .header("x-runtime-requestid", &request_id)
        .body(Body::from_stream(body_stream))
        .unwrap_or_else(|_| StatusCode::INTERNAL_SERVER_ERROR.into_response())
}
...
mod tests {
    ...
    fn fake_server_detects_local_upstream_eventstream_by_accept_and_target() {
        ...
        target_headers.insert("x-account-runtime-fixture", "eventstream".parse().unwrap());
    ...
    fn fake_server_detects_model_discovery_and_reports_reasoning_schema() {
        ...
        cli_headers.insert("x-account-runtime-fixture", "models".parse().unwrap());
    ...
    #[test]
    fn real_upstream_guard_accepts_primary_env() {
        unsafe {
