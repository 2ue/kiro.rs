...
const normalizeExternalPoolRouteMode = (value?: string): ExternalPoolsConfig['externalPoolRouteMode'] => {
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-3 md:grid-cols-5">
            ...
            <SummaryItem label="入口策略" value={fallbackActive || directPolicyActive || routePolicyActive ? '已配置' : '未配置'} />
          ...
          <PolicyBlock
            title="1.1 路由策略"
            active={externalEnabled}
            description="控制哪些入口进入本地凭证或外部池。"
          >
            <div className="grid gap-4 lg:grid-cols-2">
              <FormSection title="本地凭证路由" description="决定哪些入口允许进入本地凭证；其余入口会按外部池策略处理。">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  >
                    <option value="allow_all">全部入口允许进入本地凭证</option>
                    <option value="allow_list">只允许下列入口进入本地凭证</option>
                    <option value="deny_list">禁止下列入口进入本地凭证</option>
                  </SelectBox>
                  <TextArea
              ...
              <FormSection title="外部池路由" description="决定哪些入口允许进入外部池；默认允许全部入口。">
                <div className="grid gap-3 sm:grid-cols-2">
                  <SelectBox
                    disabled={!externalEnabled}
                    label="路由模式"
                  ...
                  >
                    <option value="allow_all">全部入口允许进入外部池</option>
                    <option value="allow_list">只允许下列入口进入外部池</option>
                    <option value="deny_list">禁止下列入口进入外部池</option>
                  </SelectBox>
                  <TextArea
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <FormSection title="显式直连" description="开关打开即全量直连外部池；模型和路径规则只用于细分记录的直连原因。">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-5">
                  <Toggle label="启用本地熔断统计" checked={configDraft.localPoolCircuitEnabled} onChange={(localPoolCircuitEnabled) => setConfigDraft((prev) => ({ ...prev, localPoolCircuitEnabled }))} />
                  <NumberBox disabled={!configDraft.localPoolCircuitEnabled} label="熔断窗口秒数" value={configDraft.localPoolCircuitWindowSecs} min={1} onChange={(localPoolCircuitWindowSecs) => setConfigDraft((prev) => ({ ...prev, localPoolCircuitWindowSecs }))} />
                  <NumberBox disabled={!configDraft.localPoolCircuitEnabled} label="失败次数阈值" value={configDraft.localPoolCircuitOpenAfterFailures} min={1} onChange={(localPoolCircuitOpenAfterFailures) => setConfigDraft((prev) => ({ ...prev, localPoolCircuitOpenAfterFailures }))} />
                  <NumberBox disabled={!configDraft.localPoolCircuitEnabled} label="涉及凭证数" value={configDraft.localPoolCircuitRequireDistinctCredentials} min={1} onChange={(localPoolCircuitRequireDistinctCredentials) => setConfigDraft((prev) => ({ ...prev, localPoolCircuitRequireDistinctCredentials }))} />
                  <NumberBox disabled={!configDraft.localPoolCircuitEnabled} label="熔断秒数" value={configDraft.localPoolCircuitOpenSecs} min={1} onChange={(localPoolCircuitOpenSecs) => setConfigDraft((prev) => ({ ...prev, localPoolCircuitOpenSecs }))} />
                </div>
              </FormSection>
            </div>
          </PolicyBlock>
          ...
          <NumberBox disabled={!externalEnabled} label="连续失败计数（兼容）" description="保留旧配置字段；普通瞬态失败只降低调度优先级，不会触发池级冷却。池级冷却仅用于模型不可用、端点错误或自动禁用等极端状态。" value={configDraft.externalPoolTransientFailureCooldownThreshold} min={0} onChange={(externalPoolTransientFailureCooldownThreshold) => setConfigDraft((prev) => ({ ...prev, externalPoolTransientFailureCooldownThreshold }))} />
