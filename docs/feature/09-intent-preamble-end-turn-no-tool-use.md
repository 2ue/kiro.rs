...
- [ ] 单测：`end_turn + 无 tool_use + 有工具定义 + 有文本` → 命中标记。
- [ ] 单测：`tool_use` 轮 / 纯 thinking 轮 / 无工具定义轮 → 不命中。
- [ ] 诊断字段仅出现在 `data`，不改变 SSE 输出与 stop_reason。
- [ ] 现网按会话查询能聚合出该 flag。

## 关联

- 代理/上游各类错误归档：`docs/feature/README.md`。
- 流式终止路径：`docs/feature/02-stream-upstream-idle-timeout.md`（同为流式，但那是真超时错误，本文是正常 end_turn）。
