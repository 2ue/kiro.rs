# Account Runtime 上游"签名"与设备指纹全链路分析
...
**本项目对 Account Runtime/CodeWhisperer 上游不存在 AWS SigV4 请求签名。** `Cargo.toml` 无 `aws-sigv4` / `aws-sig*` / `hmac` 直接依赖；`src` 下无 `x-amz-date` / `x-amz-content-sha256` 生成逻辑。上游鉴权靠 `Authorization: Bearer <token>`，AWS JSON 1.0 + event-stream 只是协议壳。
...
3. 按凭据类型派生，两条路径互斥不回落（`machine_id.rs:69-82`）
   - API Key 凭据：`sha256("Account RuntimeAPIKey/" + apiKey)`
   - OAuth 凭据：`sha256("KotlinNativeAPI/" + refreshToken)`
4. 随机兜底 `sha256("Account RuntimeFallback/" + uuid_v4)`，按 `credentials.id` 进程内缓存（`machine_id.rs:84-108`）
...
| --- | --- | --- | --- |
| IDE API / MCP | `aws-sdk-js/1.0.34 ... api/codewhispererstreaming#1.0.34 m/E Account RuntimeIDE-{ver}-{mid}` | 是 | `endpoint/ide.rs:57-64` |
...
| usage_limits | `aws-sdk-js/1.0.0 ... api/codewhispererruntime#1.0.0 m/N,E Account RuntimeIDE-{ver}-{mid}` | 是 | `refresh.rs:1041-1055` |
| IdC refresh | `aws-sdk-js/3.980.0 ... api/sso-oidc#3.980.0 m/E Account RuntimeIDE` | 否 | `refresh.rs:837-857` |

另有两条 refresh 简化 UA：social refresh 为 `Account RuntimeIDE-{ver}-{mid}`（`refresh.rs:679-684`），external IdP refresh 为 `Account RuntimeIDE-{ver}`（`refresh.rs:764`）。
...
- 出站：Account Runtime `ReasoningContentEvent.signature` 下发为 Anthropic `signature_delta`（`src/anthropic/stream.rs:2522-2533`）或非流式 `signature`（`src/anthropic/handlers.rs:8496-8516`）
...
- `src/bin/account_runtime_loadtest.rs:1932-1943` 的 `x-amz-security-token`：fake server 抓包的敏感头脱敏名单，不是生产请求生成
...
- Claude Code CLI 不直连 Account Runtime，它打 Anthropic `/v1/messages` 用 `x-api-key` 或 OAuth Bearer，同样无 SigV4。仓库内可确认的是本项目提供 `/cc/v1` 兼容路由（`src/anthropic/router.rs:321-345`），入站认证支持 `x-api-key` 或 Bearer（`router.rs:154-158`、`common/auth.rs:44-66`），再转换到 Account Runtime endpoint 并构造 Account Runtime IDE / Amazon Q CLI 形状 UA
- 官方 Account Runtime IDE 的 machineId 是真实设备 ID（每台机器不同，通常来自 VS Code machineId），而本项目是从凭据材料哈希出的稳定值
