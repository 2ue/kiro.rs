...
- Record the post-release documentation commit for `v0.0.114` without moving the published tag; then move to post-release observation.
...
- Carry the now-passing r8 real Claude Code CLI 5x20/5x100 long-session/resume gate into native Account Runtime upstream and MCP/search/image/agent/fault matrices: internal transcript markers are forbidden, but arbitrary user/tool_result fixture text is not itself a leak.
...
- Carry the development thinking wire pass (`max` retained through converter/provider) and generic Bash/Read long-session pass into real-upstream delta/usage and active/passive thinking long-session matrices; do not require or probe an unadvertised Account Runtime `thinking` field.

## Next

- Post-release observation: watch for protocol leak, scheduler Redis degraded fallback, usage/RPM amplification and build-artifact recurrence samples; collect only read-only, redacted production evidence if needed.
...
- This plan's former root report now lives in [the runtime-correctness feature workspace archive](../../../archive/runtime-correctness-feature-workspace-history/final-report.md). Do not use the archived `NO-GO` wording as current state; current scheduler/runtime follow-up lives in the [Rust Runtime Scheduler Stabilization](../rust-runtime-scheduler-stabilization/README.md) plan.
