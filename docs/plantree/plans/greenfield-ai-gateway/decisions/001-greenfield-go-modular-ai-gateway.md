...
The current system contains successful Account Runtime, Anthropic and Claude Code behavior, but its architecture concentrates unrelated responsibilities in large state and service authorities. Examples include `src/storage/postgres.rs` at more than 11,000 lines, `src/local_upstream_impl/token_manager/manager.rs` at more than 8,000 lines, `src/anthropic/handlers.rs` at more than 7,000 lines, and `src/local_upstream_impl/provider.rs` at nearly 4,000 lines as of this decision.

The earlier modernization plan proposed a complete Rust rewrite with two retained frontends and a fixed 50-module target. The user has replaced that target with a new Go system whose primary product is a general AI model gateway. Account Runtime must be fully supported in the first release but must not define the generic architecture.

Future upstream providers or API gateway integrations may require scheduling, account allocation, usage extraction, protocol conversion and retry behavior similar to Account Runtime. They may also require different implementations. Sharing concrete algorithms is therefore optional; sharing typed semantics and lifecycle contracts is mandatory.
...
5. Make Account Runtime a vertical provider module that privately owns Account Runtime authentication, endpoints, account scheduling, transport, EventStream decoding, models, quota, errors and raw usage extraction.
...
- Account Runtime-specific DTOs, credentials, errors, Redis keys and scheduling state cannot appear in generic gateway packages.
- Generic behavior must be proven with at least one non-Account Runtime contract implementation, not inferred from a Account Runtime-only design.
