...
| 2026-09-16/17 | `sub2api-kiro-protocol-interop-p0-20260916.md` | Claude Code/Anthropic ↔ Kiro P0 parser, typed event shape, tool input/name mapping, Anthropic SSE lifecycle, isolated real local credentials, fake external-pool protocol, model naming contract and leak-runner evidence contract | Full Rust `2113/0/6 ignored` plus `kiro_loadtest 31/31`; frozen binary SHA recorded; real Claude CLI `2.1.273` normal/alias/usage closed through local credentials; fake external stream/non-stream passed; real thinking/tool-use partial due `402 quota exhausted`; cleanup and artifact inventory passed. Follow-up contracts added Claude Code external-pool model canonicalization and minimal stdout hash/redacted-suspicious-line evidence for long-session leak review; full dynamic long-session rerun still pending |
