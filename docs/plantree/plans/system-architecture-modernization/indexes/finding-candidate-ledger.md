...
| `CAND-20260712-005` | 2026-07-12 | Promoted | Account Runtime/external non-stream/error paths can collect complete unbounded response bodies | `MOD-ACCOUNT_RUNTIME-UPSTREAM`, `MOD-EXTERNAL-UPSTREAM`; before R5 integration | Retract only if every reader enforces incremental accepted limits | `RES-003`; fixtures `R0.7/R0.8`, final `R5.1/R5.2` |
...
| `CAND-20260712-010` | 2026-07-12 | Promoted | `Account RuntimeProvider::client_for` uses an unbounded proxy-keyed client map retaining rotated proxy identities/secrets | `MOD-ACCOUNT_RUNTIME-UPSTREAM`; before R5.1 integration | Retract only with cardinality/age/invalidation/secret-release proof under churn | `RES-004`; fixtures `R0.7`, final R5.1 |
