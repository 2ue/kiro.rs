...
| [Account Runtime Upstream Accounts](plans/account-runtime-upstream-accounts/README.md) | In Progress | Current Rust refactor target: account-named Admin/API/runtime config/facade boundaries are landing while compatibility aliases preserve existing callers; remaining work is migrating internals away from legacy external-pool and Kiro-specific runtime paths | 2026-08-15: handler payload guard/diagnostic call sites, local body typed request boundary, stream event entrypoint and runtime diagnostics now use local-upstream wording, JSON stream error-envelope usage diagnostics avoid retaining provider messages and use neutral official-upstream source labels, and payload/body/stream/signature/account-only checks pass; earlier account usage, route subtype/reason, fallback context, body/model/retry diagnostics, official upstream envelope and warning-header work remains landed; old Kiro metering, `external_*` compatibility copies and legacy warning header remain for existing records/clients | Continue migrating runtime internals behind `account_runtime`, then remove Kiro provider/body/envelope/event dependencies once generic scheduler/proxy/usage behavior has account-owned names |
