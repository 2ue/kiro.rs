...
| [Account Runtime Upstream Accounts](plans/account-runtime-upstream-accounts/README.md) | In Progress | Current Rust refactor target: account-named Admin/API/runtime config/facade boundaries are landing while compatibility aliases preserve existing callers; remaining work is migrating internals away from legacy external-pool and Kiro-specific runtime paths | 2026-08-14: frontend account boundaries, account migration bridge, account-owned Admin DTOs, manager wrappers, config/request-state fields, Admin account bridges, account-runtime config extension/validation, account record aliases, account storage/runtime/Admin HTTP bridges, Anthropic account-route aliases and account manager scheduling wrappers landed; scoped Rust checks and `pnpm --dir ui check` passed | Continue migrating runtime internals behind `account_runtime`, then remove Kiro provider/body/envelope/event dependencies once generic scheduler/proxy/usage behavior has account-owned names |
