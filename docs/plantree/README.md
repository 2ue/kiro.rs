...
| --- | --- | --- | --- | --- |
| [Rust Runtime Scheduler Stabilization](plans/rust-runtime-scheduler-stabilization/README.md) | In Progress | Current Rust runtime/scheduler production stabilization: external-pool HA and stream retry releases are complete; passive quality-aware scheduling is focused-validated for kiro.rs; broader architecture, candidate observability, thinking signature, sub2api parity and production observation remain active | 2026-09-15: external-pool passive quality sampling, hard priority tiers, Top-K scoring, probation/probe/recovery, degradation-window handling, stream failure feedback and UI status contracts verified; see [implementation status](plans/rust-runtime-scheduler-stabilization/implementation-status.md) and [quality-aware scheduling summary](plans/rust-runtime-scheduler-stabilization/topics/quality-aware-scheduling-summary.md) | Perform read-only production observation for released candidates, then continue sub2api quality settings/feedback and independent scheduler observability follow-ups |
