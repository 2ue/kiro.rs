...
| --- | --- | --- | --- | --- |
| [Rust Runtime Scheduler Stabilization](plans/rust-runtime-scheduler-stabilization/README.md) | In Progress | Current Rust runtime/scheduler production stabilization: local-account WebSearch and tool parsing focused fixes verified; scheduler architecture, cooldown policy, thinking signature, validation gates, issue-status governance and document disposition remain active | 2026-07-31: native `web_search_YYYYMMDD`, mixed native WebSearch server-side execution, Claude CLI `WebSearch`, tool-name/schema-key mapping, and tool-result-only follow-up fixes recorded in [implementation status](plans/rust-runtime-scheduler-stabilization/implementation-status.md); [current issue status index](../../feature/issues/current-issue-status-index-20260731.md) and [issue status governance](plans/rust-runtime-scheduler-stabilization/indexes/issue-status-governance.md) are active | Keep issue-status rollup current while finishing Wave 1 image-source matrix, model-reporting clarity, broader multi-tool/history regressions, first documentation archive batch, and scheduler/external-pool chaos matrix |
