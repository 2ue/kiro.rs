...
| --- | --- | --- | --- | --- |
| [Rust Runtime Scheduler Stabilization](plans/rust-runtime-scheduler-stabilization/README.md) | In Progress | Current Rust runtime/scheduler production stabilization: external-pool HA and stream retry releases are complete; passive quality-aware scheduling and Claude Code/Kiro P0/P1 interop are focused-validated; broader architecture, candidate observability, thinking signature and production observation remain active | 2026-09-18: current HEAD `09c0021` adds payload trim/pairing metadata fixture, final fail-closed tool-pairing invariant, opaque redacted thinking handling and generic public error mapping; batch import/refresh selected positive-quota accounts, and real C1/C2 completed Sonnet 4.5/Haiku 4.5 success, thinking, tool-use, usage and SSE checks on `19023`; C0 passed `2129/0/6 ignored` plus `kiro_loadtest 31/31`, release build, diff and artifact inventory | Verify the remaining real third-party pool discovery/runtime combination; keep schema profile, thinking multi-block and WebSearch index work behind independent evidence |
| [Cross-System Configuration Sync](plans/cross-system-config-sync/README.md) | Planning | Cross-system sync for local accounts, external pools, proxies, runtime strategy and selected catalogs; receiver-controlled acceptance, strict version gating, JSON import records and lossless pre-import restore are hard requirements | 2026-09-23: initial architecture and import safety requirements recorded after `v0.0.169` publication attempt | Review and implement the version-gated import/export protocol, receiver acceptance modes, pre-import snapshots and restore workflow before enabling automatic acceptance |
