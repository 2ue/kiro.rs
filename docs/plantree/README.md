...
| --- | --- | --- | --- | --- |
| [Rust Runtime Scheduler Stabilization](plans/rust-runtime-scheduler-stabilization/README.md) | In Progress | Current Rust runtime/scheduler production stabilization: local-account WebSearch and tool parsing focused fixes verified; external-pool body-mode/model routing plus configurable same/cross-pool retry and cooldown recovery are focused-verified; scheduler architecture, candidate observability, thinking signature, validation gates, issue-status governance and document disposition remain active | 2026-08-04: external-pool body-mode/model routing, default `anthropic-version`, same/cross-pool retry budget separation, Retry-After cooling, consecutive transient cooldown escalation and cooldown-clear recovery recorded in [implementation status](plans/rust-runtime-scheduler-stabilization/implementation-status.md); [current issue status index](../../feature/issues/current-issue-status-index-20260731.md) and [issue status governance](plans/rust-runtime-scheduler-stabilization/indexes/issue-status-governance.md) are active | Keep issue-status rollup current while finishing candidate rejection observability, model-field display clarity, first documentation archive batch, and scheduler/external-pool chaos matrix |
