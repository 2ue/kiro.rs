...
| --- | --- | --- | --- | --- |
| [Rust Runtime Scheduler Stabilization](plans/rust-runtime-scheduler-stabilization/README.md) | In Progress | Current Rust runtime/scheduler production stabilization: external-pool HA and stream retry releases are complete; passive quality-aware scheduling and Claude Code/Kiro P0 interop are focused-validated; broader architecture, candidate observability, thinking signature and production observation remain active | 2026-09-16/17: Claude Code/Kiro P0 focused contracts, isolated real local-credential normal/alias/usage loop, fake external-pool SSE lifecycle, external-pool model-name canonicalization and CLI leak-runner evidence contract validated; see [implementation status](plans/rust-runtime-scheduler-stabilization/implementation-status.md) and [P0 interop evidence](../../feature/evidence/sub2api-kiro-protocol-interop-p0-20260916.md) | Rerun full long-session dynamic gate with the new leak evidence, then rerun real thinking/tool-use with a credential that has confirmed quota |
