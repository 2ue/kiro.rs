...
| [Account Runtime Upstream Accounts](plans/account-runtime-upstream-accounts/README.md) | In Progress | Current Rust refactor target: account-named Admin/API/runtime config/facade boundaries are landing while compatibility aliases preserve existing callers; remaining work is migrating internals away from legacy external-pool and Kiro-specific runtime paths | 2026-08-14: usage APIs expose upstream metering fields and new upstream account route subtypes/reasons (`account_*` / `local_rescue_after_account`), Anthropic fallback routing uses `AccountFallbackContext`, payload guard request-runtime/wrapper naming is account-oriented, parsed body plans and body/model/retry pipeline diagnostics use local-upstream/account names, official upstream error-envelope/helpers and warning headers use account-neutral names, and runtime UI wording uses upstream-account routing; old Kiro metering, `external_*` compatibility copies and legacy warning header remain for existing records/clients; scoped Rust checks, focused envelope/body/stream/handler/usage tests and UI type checks passed | Continue migrating runtime internals behind `account_runtime`, then remove Kiro provider/body/envelope/event dependencies once generic scheduler/proxy/usage behavior has account-owned names |
