...
| 编号 | 问题 | 状态 | 修复点 | 验证 |
| --- | --- | --- | --- | --- |
| P1 | 现网从 113 升 114 后备用池看似丢失、Dashboard 报 `error returned from database` | 已修复/已加防复发保护 | `PostgresStore::connect` 增加当前二进制所需 schema 兼容检查；Compose 模板显式 `ACCOUNT_RUNTIME_POSTGRES_MIGRATE_ON_START=true`；Dashboard 总接口降重；旧 admin-ui 改用拆分接口 | Rust schema 单测/聚焦编译；admin-ui tsc/build；新 UI tsc/build |
...
| P6 | Claude Code CLI / `thinking` / `output_config` 兼容性分叉与 body 归一化 | 已分析并补齐回归验证 | Account Runtime-native 序列化、CLI/IDE body transform、Anthropic converter/request_facts 的组合矩阵对齐；`max` 不被压成 `high`，`thinking=disabled` 时由 native wire 侧去掉不兼容 sibling 字段 | `cargo test --bin account-runtime output_config -- --nocapture`、`cargo test --bin account-runtime thinking -- --nocapture`、真实 Claude CLI capture gate 通过 |
...
- `src/local_upstream_impl/model/requests/account-runtime`、`src/local_upstream_impl/endpoint/cli.rs`、`src/local_upstream_impl/endpoint/ide.rs`
  - CLI / IDE / Account Runtime-native 三条路径都保持 `output_config.effort` 的显式值，不把 `max` 静默压成 `high`。
...
- `docker-compose.database.yml`
  - app 环境显式增加 `ACCOUNT_RUNTIME_POSTGRES_MIGRATE_ON_START: ${ACCOUNT_RUNTIME_POSTGRES_MIGRATE_ON_START:-true}`。
...
- `cargo test --locked --all-targets postgres_schema_compatibility_check_rejects_missing_upgrade_column` 编译通过；本机无 `ACCOUNT_RUNTIME_TEST_POSTGRES_URL` 时测试体按既有约定跳过真实 PgSQL。
...
- `cargo test --bin account-runtime output_config -- --nocapture` 通过，覆盖 `max/high/low` 显式 effort、disabled thinking、native reasoning、IDE/CLI/body guard 组合。
- `cargo test --bin account-runtime thinking -- --nocapture` 通过，覆盖 signed/unsigned/redacted thinking、CLI/IDE 转换、prompt steering、payload guard、stream 事件与 provider 重试。
- `git diff --check` 通过。
