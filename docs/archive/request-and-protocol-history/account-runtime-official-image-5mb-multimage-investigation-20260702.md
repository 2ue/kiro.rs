# Account Runtime 官方图片 5MB 限制与多图方案调查
...
正确做法是：把原始大图先处理成多张独立、合法、可打开的图片，每张图片重新编码后都要小于官方限制，然后按多图请求发给 Account Runtime 官方上游。
...
当前判断图片大小时，看的是 Account Runtime 图片里的 `source.bytes` 长度：

```rust
fn kiro_image_source_bytes(image: &crate::kiro::model::requests::conversation::Account RuntimeImage) -> usize {
    image
        .source
        .bytes
        .as_ref()
