# Account Runtime 协议改造本地前后对比测试 Runbook
...
- 本地 API key：`config.json` 的 `apiKey`，当前为 `sk-account-runtime-local-debug`。
- `ccman cc current` 当前已经指向 `local-account-runtime-9022`，URL 为 `http://127.0.0.1:9022/cc`。
- 本机 `cc` 被 Volta 覆盖，Rust 测试/构建建议显式使用 `/usr/bin/cc`。

推荐环境变量：

```bash
export ACCOUNT_RUNTIMES_BASE_URL="http://127.0.0.1:9022"
export ACCOUNT_RUNTIMES_CC_BASE_URL="http://127.0.0.1:9022/cc"
export ACCOUNT_RUNTIMES_API_KEY="sk-account-runtime-local-debug"
...
RUST_LOG=info,account_runtime=debug cargo run -- --config config.json \
...
```bash
ccman cc add \
  --name local-account-runtime-9022 \
  --desc "local kiro.rs /cc" \
  --base-url http://127.0.0.1:9022/cc \
  --api-key sk-account-runtime-local-debug \
  --switch
```
...
```bash
curl -sS "$ACCOUNT_RUNTIMES_BASE_URL/healthz" | tee .local-run/protocol-before-after/before-healthz.json

curl -sS "$ACCOUNT_RUNTIMES_CC_BASE_URL/v1/models" \
  -H "Authorization: Bearer $ACCOUNT_RUNTIMES_API_KEY" \
...
```bash
curl -sS "$ACCOUNT_RUNTIMES_CC_BASE_URL/v1/messages" \
  -H "Authorization: Bearer $ACCOUNT_RUNTIMES_API_KEY" \
...
```bash
curl -sS "$ACCOUNT_RUNTIMES_BASE_URL/v1/messages" \
  -H "x-api-key: $ACCOUNT_RUNTIMES_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model":"sonnet",
    "max_tokens":1024,
...
```bash
claude -p --model sonnet --output-format stream-json --verbose \
  --agents '{"protocol_auditor":{"description":"审计 Account Runtime 协议风险","prompt":"你只分析，不修改文件。重点看 profileArn、MCP、Claude Code tool use。"}}' \
...
```bash
rg -n "Account Runtime API 凭据调用链路|attempt|retry|disable_and_retry|force_refresh|ListAvailableProfiles|ListAvailableModels|profileArn|request id|上游" \
...
- 服务进程：`./target/debug/account-runtime -c config.json --credentials credentials.json`
- 服务地址：`http://127.0.0.1:9022`
- Claude Code 入口：`http://127.0.0.1:9022/cc`
- `ccman cc current`：`local-account-runtime-9022`
...
- 旧 session id `387d8932-8414-42f4-a955-4d86e012ecab` 被 Claude CLI 报 `already in use`，因此最终复测改用新 session id。这是本地 Claude CLI session 状态，不是 Account Runtime 上游请求失败。
...
- Debug 日志里的全局 MCP server 噪声，如 `exa`、`context7`、`tapd_mcp_http` 初始化问题，不作为 Account Runtime 协议失败；显式 filesystem MCP 已成功。
