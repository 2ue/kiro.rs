# Account Runtime CLI 抓包协议完整性对照分析

日期：2026-07-02  
范围：只分析 `tmp/cap/kiro-cli/` 抓包与当前 `kiro.rs` 本地代码，不改生产代码。  
目的：判断当前项目在调用 Account Runtime 上游时，针对不同调用面是否还需要完善，避免只看主聊天请求而漏掉模型发现、profile 查询、telemetry 等真实 Account Runtime CLI 行为。

## 1. 总结

当前项目的主聊天链路已经比较接近抓包里的 Account Runtime CLI `GenerateAssistantResponse`。也就是说，真正影响“能不能对话”的 `runtime.us-east-1.kiro.dev` 请求，目前方向基本对。

但如果目标是更完整地贴近官方 Account Runtime CLI 的真实调用协议，当前项目还不完整。抓包里至少有四类调用面：
...
简单说：**聊天请求主链路没有明显跑偏；管理面和上报面还没按 Account Runtime CLI 抓包补齐。**
...
这说明 `GetProfile` 不是“可有可无的另一个模型列表接口”，它能拿到 profile 状态、超额配置、引用推荐配置等信息。当前项目如果以后要更像 Account Runtime CLI，或者要在后台展示/判断这些状态，就需要补这条管理面调用。

### 2.2 `ListAvailableModels`

抓包文件：

- `tmp/cap/kiro-cli/003_182700_management.us-east-1.kiro.dev:443_?origin=ACCOUNT_RUNTIME_CLI&profileAr.json`
...
| --- | --- |
| method | `POST` |
| url | `https://management.us-east-1.kiro.dev:443/?origin=ACCOUNT_RUNTIME_CLI&profileArn=...` |
| host | `management.us-east-1.kiro.dev` |
| content-type | `application/x-amz-json-1.0` |
| x-amz-target | `AmazonCodeWhispererService.ListAvailableModels` |
| body | `{"origin":"ACCOUNT_RUNTIME_CLI","profileArn":"...profile/YKXDHKXAWHQX"}` |
...
| currentMessage origin | `ACCOUNT_RUNTIME_CLI` |
| agentTaskType | `vibe` |
...
这条调用看起来不是生成回复必须依赖的接口，但它是官方 CLI 的真实上报链路。如果目标是“尽量完整复现 Account Runtime CLI 协议”，这条需要补；如果目标只是保证代理聊天可用，它不是最高优先级。
...
- 参数里有 `origin=ACCOUNT_RUNTIME_CLI`、`maxResults=50`、可选 `profileArn`、可选 `nextToken`。
...
- 不要从 Account Runtime 其他项目里引入额外 system prompt 或行为 prompt，这会改变模型行为，不是协议修复。
...
3. body 至少包含 `origin=ACCOUNT_RUNTIME_CLI` 和真实可用的 `profileArn`。
...
- 对“聊天能不能成功”不是必须。
- 对“完整贴近 Account Runtime CLI 调用协议”是缺口。
...
- 不建议为了补这条影响现有 Account Runtime token 调度。
...
但因为这批抓包没有 MCP，不能下结论说它完全对，也不能说它错。后续如果要完整判断 MCP，需要单独抓 `q.us-east-1.amazonaws.com/mcp` 或真实 Account Runtime CLI MCP 相关请求。
...
当前已经有 `CliEndpoint`，但抓包说明 Account Runtime CLI 至少还有：
...
所以“已有 CLI endpoint”只能说明 runtime 主链路被覆盖了一部分，不能说明整个 Account Runtime CLI 协议面都覆盖了。
...
- body 使用 `origin=ACCOUNT_RUNTIME_CLI` 和真实 profileArn。
...
所以这次抓包给出的关键结论不是“聊天协议整体错了”，而是：**当前项目把 CLI runtime 主链路做得比较接近，但还没有完整建模 Account Runtime CLI 的 management 和 telemetry 调用面。**
...
- `conversationState.currentMessage.userInputMessage.origin` 是 `ACCOUNT_RUNTIME_CLI`。
...
- `src/kiro/endpoint/cli.rs:183-190`、`src/kiro/endpoint/cli.rs:221-236`：把请求体里的 origin 改成 `ACCOUNT_RUNTIME_CLI`。
...
- 请求 method 是 `POST`。
- `x-amz-target` 是 `AmazonCodeWhispererService.ListAvailableModels`。
- body 里有 `origin=ACCOUNT_RUNTIME_CLI` 和 `profileArn`。
- URL query 里也有 `origin=ACCOUNT_RUNTIME_CLI` 和 `profileArn`。
