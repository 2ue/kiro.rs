# Account Runtime 上游 400 Improperly Formed Request 分析
...
该日志来自线上服务，不是本地服务日志。日志中的“非流式 API 请求失败”是当前服务调用 Account Runtime 上游后记录的错误，不是下游客户端直接返回的错误文本。

## 结论

这类错误的直接含义是：当前服务已经完成凭据调度，并使用某个凭据把转换后的请求发送给 Account Runtime 上游；Account Runtime 上游返回了 `400 Bad Request`，认为最终请求体格式不合法。
...
更准确的归因是：这是一个请求内容或协议转换问题。根因可能在下游客户端，也可能在当前服务的 Anthropic-to-Account Runtime 转换层。

单凭这一条错误日志，不能 100% 判断是下游客户端问题还是当前服务问题。它只能证明 Account Runtime 上游拒绝了当前服务发过去的最终请求体。
...
也就是当前服务已经向 Account Runtime 上游发起 HTTP 请求，并收到了非 2xx 响应。
...
下游客户端传入的是 Anthropic 兼容格式。这个格式可能对 Anthropic 官方是合法的，但 Account Runtime 上游不一定支持同样的结构。

当前服务的职责是把 Anthropic 请求转换为 Account Runtime 请求。因此：

- 如果下游传入的是明显非法 JSON、非法 messages 或非法 tools，那么更偏向下游客户端问题。
- 如果下游传入的是 Anthropic 合法请求，但当前服务转换后变成 Account Runtime 不接受的结构，那么是当前服务兼容层问题。
- 如果 Account Runtime 上游近期改变了支持格式或校验规则，那么可能是上游兼容变化导致当前服务需要适配。

所以这类错误更适合定义为“请求兼容性问题”，而不是直接定性为“客户端错”。

## 和 Invalid Model 的区别

模型不存在或模型不被 Account Runtime 支持时，常见错误类似：
...
它没有给出具体字段，也没有明确 reason。它更像是 Account Runtime 上游对请求 JSON 结构、消息历史、工具调用、content block、schema 或上下文状态不满意。
...
- `additionalProperties` 是 Account Runtime 不接受的类型。
...
Account Runtime API 要求每个工具调用关系保持一致。风险包括：
...
```rust
/// Account Runtime API 要求每个 tool_use 必须有对应的 tool_result，否则返回 400 Bad Request。
```
...
Account Runtime 上游不一定支持所有组合。尤其是以下组合需要重点观察：
...
这些请求可能在 Anthropic 语义上成立，但转换成 Account Runtime 请求后触发上游校验失败。
...
| 所有请求都失败 | 当前服务转换层、模型能力同步或 Account Runtime 上游接口变化 |
...
- 在 `src/anthropic/handlers.rs` 中识别 Account Runtime 上游返回的 `Improperly formed request`，并向下游映射为 `400 invalid_request_error`，不再落入通用 `api_error`。
- 在 `src/anthropic/payload_guard.rs` 新增最终 Account Runtime payload guard，在发送上游前按真实 JSON body 字节数检查。
...
- 在 `src/anthropic/converter.rs` 中增强孤立 tool_result 处理：兼容模式下从 Account Runtime toolResults 中移除，但将可读内容文本化追加到当前 user content，减少上下文损失。
- 递归 schema 清洗和 Account Runtime-safe tool name 映射已在 converter 中保留并有单测覆盖。
...
对于 Account Runtime 明确返回 400 且语义是请求格式错误的情况，下游响应更适合是：
...
9. 如果所有模型或所有请求都复现，排查当前服务转换层或 Account Runtime 上游接口变化。
...
> Account Runtime 上游拒绝了当前服务发出的最终请求体，错误为 `Improperly formed request`。这不是账号、额度、代理或调度问题，而是请求结构或协议转换兼容性问题。需要结合该请求的模型映射、messages、tools、tool_result、content block 和非流式路径进一步定位。当前不能仅凭该日志断定是下游客户端错误，也不能排除当前服务转换层需要增强兼容。
