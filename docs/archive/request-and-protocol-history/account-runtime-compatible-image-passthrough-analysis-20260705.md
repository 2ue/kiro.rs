# Account Runtime 兼容图片转发与外部池模式分析
...
- 上游可能是本地 Account Runtime 凭证，也可能是外部池。
- 用户期望确认是否可以“转换成 Account Runtime 兼容格式后透传图片，本地不做处理”。
...
- 第一种可以通过轻量模式实现。
- 第二种在本地 Account Runtime 凭证模式下做不到，因为 Anthropic/Claude 输入协议和 Account Runtime 上游协议不是同一个 JSON schema。
- 外部池能否接近原始透传，取决于外部池上游是 Anthropic 兼容协议还是 Account Runtime 兼容协议。
...
### Account Runtime 本地上游图片格式

Account Runtime 请求里的图片不是放在 `content[].source`，而是放在 `userInputMessage.images[].source.bytes`：
...
- `src/kiro/model/requests/conversation.rs`
  - `UserInputMessage.images: Vec<Account RuntimeImage>`
  - `Account RuntimeImage { format, source }`
  - `Account RuntimeImageSource { bytes: Option<String> }`

因此，只要上游是 Account Runtime 本地凭证，就必须把 Anthropic 图片 block 转成 Account Runtime 的 `images[].source.bytes`。这不是可选处理，而是协议转换。
...
下游 Anthropic / Claude JSON
  ...
  -> Account Runtime request JSON 序列化
...
- `src/anthropic/converter.rs`
  - `convert_image_source(...)` 将 Anthropic image source 转成 `Account RuntimeImage::from_base64(...)`
- `src/kiro/provider.rs`
  - 最终会将 Account Runtime request body 变成完整字符串/bytes 后用 reqwest `.body(body)` 发送

当前并不是纯转发，而是为了让 Account Runtime 上游能稳定识别图片，做了完整标准化。
...
- 转换后的 Account Runtime request 结构。
- 序列化后的 Account Runtime JSON body。
...
原因是 Account Runtime 本地上游不认识 Claude/Anthropic 的 `file_id`。如果下游只传 `file_id`，代理必须在某个地方能拿到真实 bytes，否则无法构造 Account Runtime 的 `source.bytes`。
...
6. `convert_image_source` 会归一化 data URL/base64，并生成 Account Runtime image。
7. token 估算可能读取图片尺寸或回退为默认图片 token。
8. payload guard 可能扫描图片 source 大小。
9. Account Runtime request 序列化会再产生完整 JSON body。
...
### 本地凭证能否做到 Account Runtime 兼容“轻量转发”
...
- 不修正 media type。
- 不缩放、不压缩、不落盘。
- 直接把下游 `source.data` 搬到 Account Runtime `source.bytes`。
- 使用 `media_type` 或 data URL metadata 推导 Account Runtime `format`。
- 缺少 media type 时直接报错，或者走默认拒绝策略，不再 decode 猜测。

不可避免的处理：

- 必须解析 JSON。
- 必须转换 Anthropic schema 到 Account Runtime schema。
- 必须重新序列化 Account Runtime JSON。
- 必须持有完整请求体或完整上游 body。

因此，对本地 Account Runtime 凭证更准确的表述是：

```text
可以做“图片不解码、不落盘的 Account Runtime schema 转换转发”；
不能做“完全不处理、不转换的图片透传”。
```
...
- 下游声明 `image/png`，实际是 JPEG，Account Runtime 上游拒绝。
- 下游 base64 非法，代理不提前发现，错误延迟到 Account Runtime 上游。
- Claude CLI 的 `count_tokens` / autocompact 估算更粗。
- payload guard 如果完全关闭，超大图片可能把本地内存或 Account Runtime 请求体打爆。
...
当前外部池并不是 Account Runtime body 模式。
...
如果外部池上游本身是 Anthropic 兼容 API，那么不应该把请求转成 Account Runtime 格式。
...
#### 2. Account Runtime 兼容外部池

如果外部池上游本身是 Account Runtime 兼容协议，那当前没有完整独立模式。

不能只把请求体改成 Account Runtime JSON，因为还要同步处理：

- Account Runtime 请求体构造。
- Account Runtime eventstream 或非流式响应解析。
- Account Runtime -> Anthropic SSE/JSON 输出转换。
- usage 字段映射。
- 错误格式统一。
- 首字延迟和流式事件顺序。
- 计费模型。
...
否则会出现请求是 Account Runtime 格式、响应却按 Anthropic 处理的错配。

因此如果要支持 Account Runtime 兼容外部池，建议新增明确协议类型：

```text
externalPool.protocol = anthropic | kiro
```

并围绕该协议类型分别处理请求和响应，而不是在现有 Anthropic 外部池逻辑里硬塞 Account Runtime body。
...
- `kiroConverted`：新增 Account Runtime 兼容外部池模式，需要完整响应转换和 usage 适配。

### localAccount Runtime.imageMode
...
1. 先增加配置模型，不改变默认行为。
2. 给外部池增加 `requestMode = anthropicRaw | anthropicNormalized`，先不要做 Account Runtime 外部池。
3. 给本地 Account Runtime 增加 `imageMode = safe | light`。
...
如果未来做 Account Runtime 兼容外部池，还需要额外验证：

- Account Runtime eventstream 到 Anthropic SSE 的事件顺序。
- 首包、message_start、content_block_start、delta、message_delta、message_stop。
- Account Runtime 错误到统一错误格式。
- usage 映射。
- 外部池模型映射与 Account Runtime model id 转换。
...
- 可以实现“不解码、不落盘的 Account Runtime schema 转换转发”。
...
```text
本地 Account Runtime：增加 light image mode，减少图片 decode 和本地处理。
外部 Anthropic：增加 raw request mode，尽量不碰图片。
外部 Account Runtime：作为独立 protocol 后续设计，不和现有外部池 Anthropic 模式混用。
```
...
Account RuntimeTyped
  转成 Account Runtime request，允许本地 Account Runtime provider 调用。
```
...
图片处理能力只挂到本地 Account Runtime 和 normalized 外部池，不挂 raw 外部池：
...
本地 Account Runtime 凭证：

```text
AnthropicTyped -> Account RuntimeTyped
...
未来 Account Runtime 官方外部上游：

```text
AnthropicTyped -> Account RuntimeTyped
response: Account Runtime -> Anthropic adapter
usage: Account Runtime usage adapter
```
...
- 本地 Account Runtime 图片 safe/light 均能处理 inline 图片；light 不 decode 图片。
- 真实 Account Runtime 凭证低并发请求通过。
...
- 未把本地 Account Runtime 凭证改成 byte-to-byte raw 透传；本地凭证仍必须做 Anthropic -> Account Runtime schema 转换。
- 未实现 `externalPool.protocol = kiro`。如果外部上游是 Account Runtime 原生协议，需要单独做请求、响应、usage、错误、流式事件适配。
- raw 外部池当前不构建 current path usage projection 上下文；如需 response-only usage project，需要后续单独拆 `usageProjectionMode`。
- light 图片模式为了避免本地下载和展开，会拒绝非 inline 的 file/remote source；inline base64/data URL 仍进入本地 Account Runtime schema 转换。
...
```text
account-runtime: 127.0.0.1:19082
...
### 真实本地 Account Runtime 凭证验证结果

临时环境：

```text
account-runtime: 127.0.0.1:19085
PgSQL: 独立临时库
...
- 普通 RGB PNG inline base64：
  - `imageProcessing.mode = safe` 返回 200，Account Runtime 上游能识别图片。
  - `imageProcessing.mode = light` 返回 200，Account Runtime 上游也能识别图片。
...
因此该失败不是 raw/body 重构导致，而是上游 Bedrock/Account Runtime 对边缘 PNG 格式不支持。测试图片应避免使用透明、
