# Account Runtime 1M 上下文、请求体阈值与 `CONTENT_LENGTH_EXCEEDS_THRESHOLD` 完整分析
...
1. 为什么模型支持 1M token 上下文，但 Account Runtime 仍可能拒绝一个约 754KB 的 JSON 请求体。
2. 这是否意味着 1M 上下文能力无效。
...
- JSON body 字节数衡量的是发送给 Account Runtime HTTP 接口的原始序列化请求体大小。
- token 上下文窗口衡量的是模型推理阶段可接受的输入 token 数量。
- JSON body 中还包含协议字段、工具定义、JSON schema、转义字符、base64 图片、PDF 提取文本、工具结果、历史消息包装和 Account Runtime 特有字段。
- 字节数和 token 数没有固定一一对应关系。

因此，不能因为 body 是 754KB 就判断模型上下文已经用满，也不能因为模型支持 1M token 就推导出 Account Runtime HTTP 入口必须接受任意小于 1MB 的 JSON body。

### 2.2 现象更像 Account Runtime 入口层或内容校验层的额外限制

现网错误由 Account Runtime 上游明确返回：
...
- 模型上下文窗口是一层限制。
- Account Runtime API 入口、网关、业务校验或内容块校验还有另一层限制。
...
从使用体验看，这种分层限制确实不理想：客户端认为 1M 上下文仍有空间，但 Account Runtime 入口提前拒绝请求。项目需要在代理层补齐兼容策略，而不是把错误简单归类为“模型上下文满了”。
...
4. Account Runtime 上游存在独立于模型上下文窗口的 payload/content threshold。

### 2.4 当前项目存在策略缺口，但没有发现明确的序列化错误

当前 `kiro.rs` 已经具备：

- Anthropic -> Account Runtime 请求转换。
- 工具名称缩短。
- JSON schema 规范化。
- `tool_use` / `tool_result` 配对修复。
- 发送 Account Runtime 前按最终 JSON body 字节数检查的 payload guard。
- 超限时从最旧 history 开始裁剪。
...
- 该请求在发送给 Account Runtime 前没有经过 history trimming。
...
1. 当前服务完成了凭据调度和模型映射。
2. 请求确实已经发送到 Account Runtime。
3. Account Runtime 上游返回了 `CONTENT_LENGTH_EXCEEDS_THRESHOLD`。
...
1. Account Runtime 官方限制固定是 450KiB、600KB、615KB 或其他常量。
...
### 4.2 HTTP 请求体和 Account Runtime payload

代理发给 Account Runtime 的 body 不只是“用户文本”。它还包括：
...
- 每个工具的 JSON schema。
- `toolResults`。
- 图片 base64 数据或 Account Runtime 图片结构。
- 文档转文本后的内容。
- `conversationId`。
- `agentContinuationId`。
- `agentTaskType`。
...
Claude Code、MCP、文件读取、日志查看、网页抓取和命令输出很容易产生大段工具结果。对模型来说这些内容可能仍处于 1M token 窗口内，但 Account Runtime 入口可能先按字节或字段长度拒绝。
...
```text
754KB > Account Runtime 上下文，所以失败。
```
...
```text
模型可能仍有大量 token 空间，
但 Account Runtime 请求入口或内容校验层提前拒绝了当前 payload。
...
3. 将 Anthropic messages 转换为 Account Runtime `ConversationState`。
4. 构造 `Account RuntimeRequest`。
5. 调用 `guard_kiro_request`。
6. provider 根据凭据和 endpoint transform body。
7. 调用 Account Runtime 上游。
8. 把 Account Runtime 响应映射回 Anthropic 兼容响应。
...
- image 转为 Account Runtime image。
...
当前 converter 会把系统提示转换到 Account Runtime 可接受的对话结构中。系统提示本身可能较长，Claude Code 还会携带运行环境、工具描述和其他上下文。
...
这是为避免 Account Runtime `Improperly formed request` 所需的兼容处理。
...
当前 converter 会把当前 user message 中的 tool result 转为 Account Runtime tool results。
...
这保证了能力不被静默削弱，但也意味着大多模态请求可能继续触发 Account Runtime 上游阈值。
...
> `460800` 是当前项目采用的保守本地阈值，不是已经证明的 Account Runtime 官方统一限制。
...
随后继续把请求发给 Account Runtime，而不是本地拒绝。

该行为符合此前确定的产品约束：

> 如果超限来自当前 user message、当前 tool result、PDF 或图片，不要默认本地拒绝或静默截断；让 Account Runtime 返回真实错误。
...
```text
标记 still_oversized 后继续发送给 Account Runtime
```
...
```text
Account Runtime 入口拒绝 754KB payload
```

甚至在 CLI 正确认识 1M 后，长会话会保留更多上下文，更容易触发 Account Runtime 入口 payload threshold。因此必须同时具备代理侧 payload shaping。
...
~/Desktop/procode/Account Runtime-Go
...
| `Account Runtime-Go` | 未发现通用 guard | 无通用策略 | 无通用策略 | 无通用策略 | 无 | 无 | 可减少系统提示噪声，但不保证解决 |
...
- 文档注释明确提到 Account Runtime 会对较大 payload 返回误导性 `Improperly formed request`。
...
- 有服务端历史摘要压缩实现。
- 消息达到一定数量后，保留最近消息，把更旧消息发给 Account Runtime 模型生成摘要。
...
- 如果 Account Runtime 实际在约 600KB 级别拒绝，754KB 不一定触发第二轮 adaptive compression。
...
- 支持 tool search，把 deferred tools 留在代理侧，需要时再发送给 Account Runtime。
...
- 把 Account Runtime 入口阈值误当成模型 token window。
...
4. 优先压缩旧历史和可确认冗余。
5. 用真实 Account Runtime JSON body 字节数做预算。
6. 阈值是经验安全预算，不宣称为 Account Runtime 官方统一上限。
...
2. provider 调用 Account Runtime 前记录：
...
继续发送 Account Runtime
让 Account Runtime 返回真实错误
```
...
4. 不依赖 CLI compact 解决 Account Runtime payload threshold。
5. 对 1M 模型单独监控：
...
| 维度 | 示例 |
| --- | --- |
| endpoint | Account Runtime IDE endpoint、其他已支持 endpoint |
...
| `still_oversized=true` | guard 已尝试处理但仍超过本地经验预算；默认继续发 Account Runtime |
...
```json
{
  ...
  "error": {
    ...
    "message": "Account Runtime upstream rejected the request because input content length exceeded its request threshold. This limit is separate from the model context window. Reduce oversized tools, system prompt, documents, images, tool results, or conversation history."
  }
}
```
...
```json
{
  ...
  "error": {
    "type": "invalid_request_error",
    "message": "Account Runtime rejected the converted request as improperly formed. Check model mapping, tool_use/tool_result pairing, tool schema, multimodal sources, and payload size."
  }
}
```
...
- 记录 Account Runtime context usage event。
...
### 17.1 不要把本地 450KiB 写成 Account Runtime 官方上限
...
CLI compact 不能解决当前单条大输入，也不能保证处理 Account Runtime 入口层 body threshold。
...
3. 保持当前内容有损处理默认关闭；未显式开启或开启后仍无法 fit 时继续 `still_oversized` 透传，让 Account Runtime 返回真实错误。
...
../Account Runtime-Go/proxy/translator.go
```
