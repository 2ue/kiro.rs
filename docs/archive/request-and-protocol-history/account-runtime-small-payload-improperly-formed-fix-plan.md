# Account Runtime 小 payload `Improperly formed request` 根因分析与修复方案

## 1. 文档目的

本文档用于独立说明一次现网 `Account Runtime rejected the converted request as improperly formed` 错误的背景、证据、原因分析和修复方案。
...
```text
/root/docker-compose/account-runtime-2ue-59137
```

服务容器：

```text
account-runtime-2ue-59137-app
```

compose 中的镜像配置为：

```text
ghcr.io/2ue/account-runtime:latest
```

但容器内实际运行版本为：

```text
account-runtime 0.0.33
```
...
这点很关键：最新版没有改变 Anthropic -> Account Runtime 的核心转换形态，也没有改变 payload guard 的协议修复规则。因此，对小 payload malformed 请求，单纯升级到 `v0.0.35` 不等于已经修复。
...
主错误来自 Account Runtime 上游。当前服务已经完成 Anthropic 请求解析、模型映射、凭据调度、Anthropic -> Account Runtime 请求转换，并向 Account Runtime 上游发出请求。Account Runtime 上游返回：
...
```text
Account Runtime rejected the converted request as improperly formed...
```
...
排查时应以 Account Runtime 返回的 `400 Bad Request {"message":"Improperly formed request.","reason":null}` 作为主因。
...
这些字段说明：该请求没有被 payload guard 裁剪、压缩、移除 orphan tool 或截断内容。请求体原样发送给 Account Runtime，上游返回 malformed。
...
该模式更像“稳定的请求形态不被 Account Runtime 接受”，不是偶发网络问题，也不是某一次请求刚好超限。
...
如果凭据整体不可用，通常会看到认证、额度、禁用、401、403、402、凭据不可调度等错误。当前错误是 Account Runtime 对请求体返回 400，因此不应优先归因于凭据。
...
并带有 Account Runtime 上游 JSON 错误体。因此不是网络层无响应。

### 7.6 不应通过切换账号重试解决

`Improperly formed request` 表示请求体形态不合法。对同一个坏 body 切换账号重试，通常仍会被 Account Runtime 拒绝，还会污染更多凭据的错误统计。
...
直接原因是：Account Runtime 上游认为转换后的 Account Runtime request body 格式不合法，返回：
...
最高概率方向是：该请求的 Anthropic -> Account Runtime 转换结果中，某些工具定义、工具 schema、工具结果、消息历史或 current message 组合触发了 Account Runtime 的严格校验。
...
这说明请求在代理现有规则下看起来是“可发送”的，但 Account Runtime 上游仍拒绝。问题很可能在 Account Runtime 更严格或未公开的校验规则中。

### 8.3 为什么不能精确指出具体字段

当前 usage 记录没有保存完整 Account Runtime request body，也没有保存完整工具 schema、映射后工具名列表或 schema 特征摘要。

Account Runtime 上游返回：
...
该逻辑的含义是：如果 malformed 其实是“过大请求被 Account Runtime 泛化成 malformed”，并且 body 超过 retry 阈值，则把它当作 size 类问题处理。
...
3. 新增 small malformed 的一次 Account Runtime-safe retry。
...
当前最大问题是：Account Runtime 返回 `reason:null`，而 usage 记录没有保存足够的结构信息。
...
没有这一步，后续只能根据 payload breakdown 推断，无法确认到底是空 content、工具 schema、工具名、历史顺序还是 Account Runtime 模型校验差异。
...
空格不会改变用户语义，只是避免 Account Runtime 对空字符串 content 的潜在拒绝。
...
## 14. 方案 C：新增 small malformed 的 Account Runtime-safe retry
...
3. 对工具 schema 使用 strict Account Runtime-safe sanitizer。
4. 压缩工具 description。
5. 检查并修复映射后工具名重复。
...
如果 retry body 已经改变，可以使用同一个已调度凭据重试一次。这样可以验证“修复后的请求形态”是否被 Account Runtime 接受，同时避免污染其他账号。

### 14.5 为什么只重试一次

避免 retry loop。

如果 Account Runtime-safe retry 仍然返回 malformed，应把失败记录为：
...
## 15. 方案 D：strict Account Runtime-safe schema sanitizer

### 15.1 背景

现有 `normalize_json_schema` 已经会清洗一部分 MCP/OpenAPI/Zod schema，但 Account Runtime 对工具 schema 的真实接受范围可能更窄。
...
strict sanitizer 不是为了保持最完整 schema，而是为了让 Account Runtime 接受请求。

建议目标：

```text
把工具 input_schema 降级到 Account Runtime 最稳定接受的 JSON Schema 子集。
```
...
这样正常请求保持原有能力，只有 Account Runtime 已经拒绝时才降级。

## 16. 方案 E：工具名重复与映射预检

### 16.1 背景

Account Runtime 通常要求工具名有效且唯一。当前代码有工具名 sanitize/shorten/hash 逻辑，但仍应显式检查映射后的工具名是否重复，尤其是大小写不敏感重复。
...
mapped_name 是否为空
mapped_name 是否符合 Account Runtime 命名规则
...
其中 `payloadGuardMaxBytes = 563200` 对 Account Runtime 实际限制偏乐观。很多请求虽然低于该阈值，仍然被 Account Runtime 返回 too-long。
...
`req_01bGG677KX44sBJfcoW6ycQt` 的主因是 Account Runtime 上游拒绝转换后的请求体：
