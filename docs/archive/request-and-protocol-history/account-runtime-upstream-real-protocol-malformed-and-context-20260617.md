# Account Runtime upstream protocol malformed/context analysis - 2026-06-17

## Scope

This note records the current local analysis and code changes for Account Runtime upstream protocol compatibility. It focuses on:

- `400 Bad Request {"message":"Improperly formed request.","reason":null}`
- Claude/Account Runtime tool-use transcript shape
- Account Runtime `contextUsageEvent` percentage based token reconstruction
...
```text
GET http://127.0.0.1:9022/healthz -> {"service":"account-runtime","status":"ok"}
```

Service restarted with rebuilt local binary:

```text
./target/debug/account-runtime -c config.json --credentials credentials.json
PID: 3007
...
This duplicate-id test is intentionally synthetic, but it matches a real malformed-risk class: a set-based checker can treat reused ids as globally paired, while Account Runtime expects adjacent tool-use/tool-result structure. The final body repair renamed the later duplicate tool-use id and rewrote its adjacent current tool result before sending to Account Runtime.

These tests prove the local free Sonnet path can complete normal text, tool-use, tool-result, and a duplicate-id repair scenario against real Account Runtime upstream. They do not prove every large/malformed production transcript is fixed; they validate that the fixed paths preserve normal behavior and eliminate one concrete malformed request class.

## Context Window Findings

Account Runtime has two separate limits that must not be mixed:

1. Model context window, reported by Account Runtime model capabilities and surfaced indirectly through `contextUsageEvent.contextUsagePercentage`.
...
The code already uses Account Runtime model catalog data first:
...
- If Account Runtime `ListAvailableModels` says the resolved upstream model is 200K, use 200K.
- If Account Runtime `ListAvailableModels` says the resolved upstream model is 1M, use 1M.
...
This explains why local free Sonnet cannot be assumed to support 1M. Other accounts may have `auto` or dot-minor `claude-sonnet-4.6` with 1M, but that must come from the real Account Runtime catalog.
...
is a Account Runtime request-body validation failure. It is not necessarily caused by local token counting. It can be caused by:
...
- empty Account Runtime `content` when only tool results or tool uses exist
...
Using only a `HashSet`, the second assistant tool use can appear valid because `X` existed earlier. Account Runtime can reject such a transcript as improperly formed.

There is a second edge: after conversion, the final Anthropic user message is removed from history and becomes Account Runtime `currentMessage`. Therefore the immediately previous assistant tool-use can be the last item in Account Runtime history. Current `tool_result` must be allowed to pair with that last assistant item, while a result for an older non-adjacent assistant must be textified or dropped from structured `toolResults`.

## Implemented Fix

`src/anthropic/payload_guard.rs` now performs final Account Runtime-body repair for:
...
This is deliberately applied after Anthropic-to-Account Runtime conversion, because it validates the actual body sent to Account Runtime, not just the original Anthropic messages.
...
- reused ids across turns are preserved long enough for the final Account Runtime payload guard to rename the later occurrence and synchronize the adjacent result
...
Provider-side Chinese label also no longer says "Anthropic 到 Account Runtime 的 payload 转换".
...
- It uses real Account Runtime model catalog context windows first, avoiding incorrect 1M/200K usage reconstruction.

Remaining risk:

- If a client sends logically contradictory duplicate tool results, the second result becomes plain text. That is preferable to sending invalid structured JSON that Account Runtime rejects.
...
```text
Service process: ./target/debug/account-runtime -c config.json --credentials credentials.json
ccman current: local-account-runtime-9022 -> http://127.0.0.1:9022/cc
...
threshold, not a Account Runtime model context-window result and not the generic upstream
...
mcp-final.stream.jsonl
                     ...
                     mcp__account-runtime-local-test__ping, received mcp-pong-final as tool_result, and
...
```text
Account Runtime upstream 429 suspicious activity temporary limits disabled several free credentials.
Claude Code default max output tokens was 32000, making free-account tests expensive.
One CLI run ended with a downstream quota/pre-deduction failure after the Account Runtime requests had
...
request 2: model selected mcp__account-runtime-local-test__ping
...
- `/Users/yuanfeijie/Desktop/procode/Account Runtime-account-manager/Account Runtime-account-manager`
- `/Users/yuanfeijie/Desktop/procode/freedom-kirors`

The comparison is about Account Runtime upstream protocol compatibility, not general code style.

### Official Claude / Claude Code Constraints Used As Reference

Account Runtime upstream is not the public Anthropic Messages API, but the observed failures map closely to Claude-style message constraints:
...
This is a direct final-assistant-prefill violation. The local converter/payload guard must ensure the final upstream Account Runtime request always has `conversationState.currentMessage.userInputMessage`.
...
### Account Runtime-account-manager Findings
...
Release-level problems solved by `Account Runtime-account-manager`:
...
- `v1.6.x` adds model capability badges derived from `ListAvailableModels`, including thinking/caching/effort metadata. This validates the decision that model capability and context window must come from the Account Runtime catalog first.
- `v1.6.x` says request headers/UA/version were aligned to official Account Runtime IDE captures, and request body includes `agentContinuationId` / `agentTaskType`. Current `kiro.rs` already has these body fields in the Account Runtime conversation state path and Account Runtime IDE-style headers in `src/kiro/endpoint/ide.rs`.
...
- `v1.6.x` simplifies thinking mode by passing through native Account Runtime reasoning events and supporting `additionalModelRequestFields`. Current `kiro.rs` already parses `reasoningContentEvent` / `messageMetadataEvent` / `ContextUsageEvent`, and maps thinking through existing converter paths.
...
- `fetchAccount RuntimeModels()` paginates `ListAvailableModels` with `origin=AI_EDITOR`, `maxResults=50`, optional `profileArn`, and `nextToken`. Current `kiro.rs` mirrors that in `IdeEndpoint::models_url()` and `Account RuntimeProvider::list_available_models_for_context()`.
...
- `translator.ts` deliberately strips request-history `reasoningContent` because sending it back to Account Runtime history can trigger `400 Improperly formed request`. Current `kiro.rs` has payload-guard support for removing history thinking blocks and native reasoning output handling.
- `translator.ts` ensures a Account Runtime current message exists and is a user message when the client ends with assistant. Current `kiro.rs` does this at the Anthropic converter and final Account Runtime payload-guard level; this is necessary for the `assistant-prefill final message is not supported` error.
- `translator.ts` preserves tool uses and tool results but relies more heavily on general `sanitizeConversation()` / `normalizeToolHistory()`. Current `kiro.rs` is now stricter: it repairs against the final Account Runtime body, renames repeated tool-use IDs by occurrence, and synchronizes adjacent tool results.

What current `kiro.rs` should keep from KAM:

- Catalog-first model capability, not alias guessing.
- Exact Account Runtime-ish endpoint/body/header shape.
...
- `0.5.9` blocks 503 retry storms from client validation errors. This is important because Bedrock/Account Runtime may report malformed tool transcripts as 5xx/503, but retrying against more credentials cannot fix it.
...
- `Account RuntimeCredentials` supports BuilderID/Social/Enterprise/IdC/API Key, per-credential regions, per-credential endpoint selection, proxy, group, and rate limit fields. Current `kiro.rs` has the same broad credential model and already migrated API keys to the correct endpoint behavior.
...
- `provider.rs` resolves Enterprise profile before calls, persists real ARN, and avoids retrying known client validation errors. Current `kiro.rs` mirrors this pattern and now labels bad requests without exposing Anthropic-to-Account Runtime conversion wording to users.
...
- Account Runtime request-body guard runs after Anthropic-to-Account Runtime conversion and validates/repairs the actual body sent upstream.
...
- Account Runtime `contextUsageEvent` is used to reconstruct input tokens when metadata usage is absent.
- 100% context usage maps to `model_context_window_exceeded`.
- Bad request labels no longer expose "Anthropic to Account Runtime conversion" language to downstream users.

Remaining risks:

- A truly oversized serialized Account Runtime body can still fail even if context percentage is below 100%; request byte limits and context-token limits are independent.
...
The purpose of testing is to compare official-Account Runtime-compatible behavior before and after the fix, not to keep branch-only test code in production.
...
- Assistant prefill / final assistant message becomes a valid Account Runtime current user message or fails as bad request without retry storm.
...
- Tool responses are structured Anthropic-compatible SSE/JSON, not leaked Account Runtime XML.
...
```text
ccman cc use local-account-runtime-9022
...
- Agent/subtask: run a CLI task likely to trigger internal task/explore behavior, but force Sonnet where possible; if Claude Code selects Haiku internally, record it as a test-environment limitation, not a Account Runtime protocol failure.
...
- Account Runtime could reject the final body as `Improperly formed request`.
...
- Current tool results are validated against the final Account Runtime body shape.
- Real local tool and duplicate-ID synthetic tests pass against Account Runtime upstream.
- User-visible errors describe request shape/tool pairing/schema/payload size, not converter internals.
- Context-window math follows the real Account Runtime catalog first.
...
3. MCP: completed in the non-`--bare` run. Claude Code used `ToolSearch` to discover
   `mcp__account-runtime-local-test__ping`, emitted a real MCP tool_use, received the MCP tool_result, and
