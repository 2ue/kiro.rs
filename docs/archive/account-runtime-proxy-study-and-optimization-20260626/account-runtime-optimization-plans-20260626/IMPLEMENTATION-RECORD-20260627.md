# Account Runtime 优化计划实现记录（2026-06-27）
...
| 04 loadtest and chaos harness | `src/bin/account_runtime_loadtest.rs` 已存在 fake server、真实上游双开关、stream/异常/恢复场景和资源采样。 | 不重复造工具；后续测试直接使用。 |
...
| 08 cachePoint and cache normalization | prompt cache 边界、volatile id normalization 已有部分实现；真实 Account Runtime cachePoint 计划此前未完成。 | 本批补齐 cachePoint 默认关闭试验能力、最终序列化注入、上游拒绝后单次无 cachePoint fallback。 |
...
### 3. 真实 Account Runtime cachePoint 试验能力
...
- `src/anthropic/converter.rs`：
  ...
  - 只对实际发送给 Account Runtime 的工具生成插入计划。
...
- 当前 Account Runtime 上游不接受 `claude-sonnet-4-6-thinking` 作为真实 `modelId`，实测返回 `INVALID_MODEL_ID`。
- 因此 `*-thinking` 请求仍映射到 Account Runtime 基础模型，但 `src/anthropic/converter.rs` 会在兼容模式下把 thinking 控制写入上游历史提示。
...
- 拿到至少一个可验证的 Account Runtime 等价 endpoint。
...
- `cargo build --locked --no-default-features --bin account-runtime --bin account_runtime_loadtest`
- `pnpm --dir ui check`
- `pnpm --dir ui build`
- `account_runtime_loadtest` 隔离代理压测：普通突发并发、thinking、显式 thinking、RPM、dfcache、异常矩阵、异常恢复、client drop、tool-use stream。
...
- 本轮没有使用真实 Account Runtime 上游账号做大并发压测，避免影响真实账号和现网状态。
- 本轮真实 Claude Code CLI 验证使用真实 CLI 客户端，但上游是隔离 fake Account Runtime server；它能验证协议、stream、thinking、tool-use、MCP、usage 字段，不等价于真实模型智商、图片识别或大文档理解质量验证。
