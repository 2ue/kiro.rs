...
- 提供一个本地可运行的压测 CLI。
- 提供一个 fake Account Runtime server，用于稳定复现异常。
...
如果项目没有 `xtask`，优先使用 `src/bin/account_runtime_loadtest.rs`。
...
## Fake Account Runtime Server 场景
...
| `invalid_tool_format` | 模拟 Account Runtime 返回 Invalid tool use format |
...
1. 新增 fake Account Runtime server。
