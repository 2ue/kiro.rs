...
本方案处理 Anthropic 消息、tool_use、tool_result、thinking、cache_control 转 Account Runtime 请求体时的格式保护和回归测试。
...
- `Account Runtime-Go/proxy/kiro_api.go`：轻量实现中有 tool 格式处理样例。
...
- 建立 tool-use 回归矩阵。
- 在发送 Account Runtime 前执行轻量 audit。
- 对可安全修复的格式进行确定性修复。
- 对不可安全修复的请求提前返回统一错误。
...
4. 缺少 `tool_use_id` 的 `tool_result` 不得发送给 Account Runtime。
5. 空 `tool_use.input` 必须规范成 `{}`，不得发送 `null`。
6. `tool_result.content` 必须规范成 Account Runtime 可接受的内容类型。
7. thinking block 不得被混入 tool input。
8. cache_control 只能在 Account Runtime 支持的位置转换；不支持的位置不得原样塞入未知字段。
...
1. 从 `converter.rs` 生成 Account Runtime body 后调用 audit。
...
- 检查 Account Runtime 不再返回 `Invalid tool use format`。
