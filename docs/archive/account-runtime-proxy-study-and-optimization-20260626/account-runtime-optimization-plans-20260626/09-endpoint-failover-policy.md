...
本方案处理 Account Runtime endpoint 多地址尝试、失败分类、重试边界、健康状态和回滚开关。
...
- 当前项目 `src/kiro/endpoint/ide.rs`：已有 native Account Runtime endpoint 处理，应在现有结构上加策略。
...
```rust
pub kiro_endpoint_candidates: Vec<Account RuntimeEndpointCandidate>
```

```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Account RuntimeEndpointCandidate {
    pub name: String,
    pub base_url: String,
    pub enabled: bool,
    pub priority: i32,
