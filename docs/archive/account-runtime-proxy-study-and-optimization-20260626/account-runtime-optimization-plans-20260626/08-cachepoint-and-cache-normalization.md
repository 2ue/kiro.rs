...
本方案处理 high-cache 路由、本地 prompt cache usage 模拟、真实 Account Runtime `cachePoint` 试验、volatile id 归一化、缓存内存边界和管理端统计。

## 来源项目与学习点

- `kiroxy/internal/reqconv/cache_points.go`：把 Anthropic `cache_control` 映射为 Account Runtime `cachePoint`。
...
- `Account Runtime-Go/proxy/cache_tracker.go` 和 `Account Runtime-account-manager/src/main/proxy/promptCacheTracker.ts`：per-account cache entry cap 思路适合补齐内存边界。
...
- 本地 high-cache 更偏 usage 模拟，不等于真实发送 Account Runtime cachePoint。
...
- 插入 cachePoint 前必须确认 Account Runtime 上游真实接受。
...
- cachePoint body 不被 Account Runtime 接受。
