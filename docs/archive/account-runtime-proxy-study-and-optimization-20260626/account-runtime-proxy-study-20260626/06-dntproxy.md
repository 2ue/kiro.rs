...
`dntproxy` 是多 provider 路由系统，不是 Account Runtime 专项生产网关。它最值得当前项目学习的不是 Account Runtime upstream，而是“选择失败原因结构化”和“fallback 边界判断”。
...
- 不进入外部池 fallback。
- 记录原始错误和 Account Runtime body 结构摘要。
...
当前项目 `Account RuntimeProvider` 也有类似多账号尝试，但建议吸收两个点：
...
- 当前项目 Account Runtime 专项协议和上游调用更完整。
