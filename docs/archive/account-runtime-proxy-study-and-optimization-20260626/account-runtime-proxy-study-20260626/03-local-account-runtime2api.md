...
本地 `kiro2api` 的价值主要在协议转换层：Anthropic/OpenAI 到 Account Runtime request 的转换、Account Runtime EventStream 到 Anthropic SSE 的转换、thinking/tag 处理、真实 cachePoint、工具结果格式。它不像当前项目那样生产化，但协议层实现有很多可学习点。

## 关键文件

| 文件 | 作用 |
| --- | --- |
| `internal/reqconv/build_payload.go` | 构建 Account Runtime payload |
| `internal/reqconv/cache_points.go` | Anthropic `cache_control` 转 Account Runtime `cachePoint` |
| `internal/reqconv/tool_results.go` | tool_result / tool_use 转 Account Runtime 结构 |
| `internal/reqconv/schema_sanitize.go` | 工具 schema 清理 |
| `internal/respconv/streaming.go` | Anthropic SSE writer |
| `internal/respconv/event_processor.go` | Account Runtime event 到内部 delta |
...
- 遍历 Anthropic tools。
- 每个 tool 对应一个 Account Runtime `ToolEntry`。
- 如果该 tool 有 `cache_control`，就在该 tool 后追加 `CachePoint{Type:"default"}`。

这说明 Account Runtime upstream 可能理解 tools 数组中的 `cachePoint` entry。当前项目现在的高缓存主要是本地 usage projection 和 prompt cache tracker，不等于真实请求 Account Runtime 建缓存。
...
- Account Runtime 对 cachePoint 的支持可能随账号类型、模型、endpoint 不一致。
...
- Account Runtime 同时输出 XML thinking 和 reasoningContentEvent 时不能双算。
- tool input JSON 不应被 max_tokens 截断成非法 JSON。
- metadata/metering 优先级要稳定。

## Tool result 格式

`internal/reqconv/tool_results.go` 把 Anthropic `tool_result` 转成 Account Runtime 工具结果时，使用：
...
这是从 Account Runtime CLI 抓包来的形态。当前项目需要确认自己的 tool result 转换是否同样兼容，尤其是 Claude Code CLI / MCP 场景：
...
- tool result 的 Account Runtime CLI 形态值得做兼容样本。
- tracing 模块边界清楚。
...
- feature flag 支持真实 Account Runtime `cachePoint`。
- 优化热点 SSE delta 写法，减少高并发分配。
- 增加 Account Runtime CLI tool_result 形态兼容测试。
