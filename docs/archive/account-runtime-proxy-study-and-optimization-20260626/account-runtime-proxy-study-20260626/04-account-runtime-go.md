# 项目分析：`Account Runtime-Go`

路径：`/Users/yuanfeijie/Desktop/procode/Account Runtime-Go`  
最新本地提交：`feb3437`，2026-06-18  
相关度：高

`Account Runtime-Go` 是一个轻量但实用的 Account Runtime 代理。它最值得学习的是 profileArn/region 处理、账号 quota/overage 判断、账号模型支持缓存，以及 OpenAI Responses API 适配。调度本身不如当前项目强。

## 关键文件

| 文件 | 作用 |
| --- | --- |
| `proxy/kiro_api.go` | Account Runtime REST/API 调用、profileArn 解析、region regionalize |
| `proxy/kiro_headers.go` | Account Runtime headers |
| `proxy/account_failover.go` | 多账号失败切换 |
| `pool/account.go` | 加权轮询、冷却、模型列表、quota/overage |
| `proxy/cache_tracker.go` | prompt cache 模拟 |
| `proxy/translator.go` | Claude/OpenAI 到 Account Runtime 转换 |
...
`Account Runtime-Go` 有 `responses_handler.go`、`responses_history.go`、`responses_store.go` 等，对 OpenAI Responses API 做兼容。
...
但这不是当前 Account Runtime 代理主线 P0。
...
- OpenAI tool call 转 Account Runtime。
- 重复 tool_result 去重。
...
- 提取 Account Runtime-Go 的 tool/result 测试样本。
