# 项目分析：`Account Runtime-account-manager`

路径：`/Users/yuanfeijie/Desktop/procode/Account Runtime-account-manager`  
最新本地提交：`447adcd`，2026-06-11  
相关度：中

`Account Runtime-account-manager` 是桌面账号管理和本地反代结合的项目。它不适合直接内置到当前服务，但账号管理体验、注册/导入、自愈、prompt cache、账号代理分桶等思路可参考。

## 关键文件

| 文件 | 作用 |
| --- | --- |
| `Account Runtime-account-manager/src/main/proxy/accountPool.ts` | 多账号轮询、断路器、指数退避、概率重试 |
| `Account Runtime-account-manager/src/main/proxy/kiroApi.ts` | Account Runtime API 调用、endpoint、headers、profileArn、agent mode、proxy |
| `Account Runtime-account-manager/src/main/proxy/promptCacheTracker.ts` | prompt cache 模拟 |
| `Account Runtime-account-manager/src/main/proxy/types.ts` | Account Runtime payload/account 类型 |
| `Account Runtime-account-manager/src/preload/index.d.ts` | 前后端账号、proxy、注册、检查、模型接口 |
...
- User-Agent 中包含 Account Runtime IDE version、SDK version、OS、Node、machineId。
