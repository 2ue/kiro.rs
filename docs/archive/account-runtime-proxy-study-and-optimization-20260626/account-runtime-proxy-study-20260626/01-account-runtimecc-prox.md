...
| `internal/kiroclient/client.go` | Account Runtime upstream HTTP client、retry、200 JSON exception 识别、OTel |
...
当前项目也有 sticky/fallback，但代码位置散在 manager 内。建议吸收 `Conductor` 这个边界：它是调度编排层，不应该关心 request 转换和 Account Runtime upstream。
...
`internal/kiroclient/idle_reader.go` 用一个 wrapper 给 `Read` 增加 idle timeout。Account Runtime 有时会返回 200 + eventstream header，但后续没有 frame，普通 client 会一直挂住。
...
这点非常重要：Account Runtime/AWS 有时会用 200 + JSON exception 表示 throttling/internal 错误。如果直接丢给 eventstream parser，会得到迷惑性的 frame parse 错误。
