...
来源：`kirocc-prox`、`ndycode/account-runtime`
...
### 4. Account Runtime malformed/tool-use 回归矩阵

来源：`9router`、本地 `kiro2api`、`Account Runtime-Go`

必须覆盖：

- 无 tools 的 follow-up，历史有 tool_use/tool_result：结构化 tool 必须 flatten。
- 有 tools 但 tool_result orphan：fold 回 user text。
- 重复 tool_use_id：rename 或去重，最终 Account Runtime body 合法。
- 空 tool_result：不能发非法空结构。
- error tool_result：按 Account Runtime 兼容格式输出。
...
### 4. 真实 Account Runtime cachePoint feature flag
...
- 插入 `cachePoint` 后真实测试 Account Runtime 接受。
- 失败自动降级。
- usage 记录是否 applied。
...
来源：`Account Runtime-account-manager`
...
- 不要在请求热路径加入概率 retry。
- 不要把多 provider registry 的复杂度搬进当前 Account Runtime 专项项目。
