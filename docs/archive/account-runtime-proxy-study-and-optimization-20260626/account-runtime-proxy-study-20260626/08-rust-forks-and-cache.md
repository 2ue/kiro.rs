...
- `/Users/yuanfeijie/Desktop/procode/kiro-research/ndycode__account-runtime`
...
| `src/kiro/provider.rs` | Account Runtime upstream 调用 |
...
- full response cache 对 Account Runtime/Claude Code 很危险。
...
## `ndycode/account-runtime`

路径：`/Users/yuanfeijie/Desktop/procode/kiro-research/ndycode__account-runtime`  
...
| 文件 | 作用 |
| --- | --- |
| `kiro/converters_openai.py` | OpenAI 转 Account Runtime |
| `kiro/converters_anthropic.py` | Anthropic 转 Account Runtime |
...
- 模拟上游集成测试：本地 fake Account Runtime server，覆盖 eventstream、200 JSON、stall、429、400。
...
`caidaoli/kiro2api`、`hnewcity/Account RuntimeaaS` 等更多是轻量部署、桌面包装、基础代理形态参考。它们对当前项目的调度和生产化帮助有限。
