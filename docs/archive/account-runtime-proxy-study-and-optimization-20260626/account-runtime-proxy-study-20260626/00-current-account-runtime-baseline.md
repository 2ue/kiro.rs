...
| Account Runtime 上游调用、多账号重试、完成 guard | `src/kiro/provider.rs` |
| Account Runtime IDE endpoint、headers、profileArn 注入 | `src/kiro/endpoint/ide.rs` |
...
| Account Runtime 尝试链路记录 | `src/kiro/call_trace.rs` |
...
这部分明显强于 `Account Runtime-Go`、`Account Runtime-account-manager`、`kiroxy`、`dntproxy` 这类更偏单机或轻量的项目。后续学习外部项目时不能把状态模型退回内存或 JSON 文件。
...
- 非流式 `Account RuntimeApiCompletion` 用 Drop 兜底，避免 body 读取中途失败导致账号长期占槽。
...
当前 `PromptCacheTracker` 通过本地 fingerprint 模拟 cache creation/read usage。这对计费和 dashboard 有用，但不等于真实向 Account Runtime upstream 发送 `cachePoint`。

外部 `kiro2api` 和 `kiroxy` 的 `ApplyToolCachePoints` 提供了真实 Account Runtime request shape 方向。当前项目可以在 feature flag 下尝试，但必须经过长会话、tool-use、Claude Code CLI、Account Runtime、OpenCode 的真实验证。
