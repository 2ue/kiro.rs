...
`9router` 不是单纯 Account Runtime 代理，而是多 provider、多协议、多客户端路由系统。它不适合当前项目整体照搬，但它在 provider registry、thinking 统一化、Account Runtime executor endpoint fallback、tool history 400 防护、stream 终止防挂方面有参考价值。

## 关键文件

| 文件 | 作用 |
| --- | --- |
| `open-sse/providers/registry/kiro.js` | Account Runtime provider registry |
| `open-sse/executors/kiro.js` | Account Runtime executor、AWS EventStream 到 SSE |
| `open-sse/config/kiroConstants.js` | Account Runtime model suffix、thinking prompt、profileArn 默认值 |
| `open-sse/translator/request/openai-to-kiro.js` | OpenAI 请求转 Account Runtime |
| `open-sse/translator/request/claude-to-kiro.js` | Claude 请求直转 Account Runtime |
| `open-sse/translator/response/kiro-to-openai.js` | Account Runtime 响应转 OpenAI |
| `open-sse/translator/response/kiro-to-claude.js` | Account Runtime 响应转 Claude |
...
`9router` 用 registry 管理 provider、model、capability、pricing、endpoint、translator。这个设计对它必要，因为它同时接 OpenAI、Anthropic、Gemini、Account Runtime、Codex、Qwen 等。

当前项目是 Account Runtime 专项代理，不应该照搬一个庞大的 provider registry。但可以学习：
...
- Account Runtime upstream event 转内部统一事件，再按目标协议输出。
...
可以学习 `9router` 的“先抽象 intent，再应用 provider format”的方式，但当前项目只需要 Account Runtime/Anthropic 子集：
...
然后再映射 Account Runtime：

- model variant 保留到 upstream。
- 或注入 Account Runtime 支持的 `<thinking_mode>` / request body 结构。
- 输出统一到 Anthropic thinking blocks。

## Account Runtime thinking prompt
...
- 这是 prompt injection，不是 Account Runtime 官方公开协议字段。
- 它可能对模型行为有影响。
- 如果 Account Runtime 已经通过模型名或 reasoningContentEvent 原生支持 think，优先保留原生模型，不要 alias 到普通模型。
...
2. 客户端传了 tools 时，把孤立 tool_result fold 回 user text，避免 dangling structured reference 触发 Account Runtime 400。
...
- 无 tools 的 follow-up 请求，历史里有 tool_use/tool_result，最终 Account Runtime body 不应保留结构化 tool。
...
- Account Runtime executor 对 endpoint/auth/fallback 的关系描述清楚。
- tool history 400 防护策略非常直接。
- stream 防挂和 terminal event 的意识强。

## 当前项目比它强的地方

- 当前项目专注 Account Runtime，生产调度能力更强。
...
- 小型 capability registry，用于 Account Runtime model/context/thinking/tool/cache 能力。
