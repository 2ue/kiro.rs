...
| `internal/kiroclient/headers.go` | native Account Runtime headers、User-Agent profile |
...
| `internal/reqconv/cache_points.go` | Anthropic `cache_control` 到 Account Runtime `cachePoint` |
...
`Policy.MinRestPeriod` 的注释非常关键：同一个账号短时间连续请求会触发 Account Runtime 风控，尤其大量账号共享同一出口 IP 时。
...
## Native Account Runtime headers

`internal/kiroclient/headers.go` 明确区分 native Account Runtime IDE shape 与 legacy shape：
...
- User-Agent 带 machine id 和 Account Runtime IDE 信息。
...
- 对 User-Agent 中的 machine id / Account Runtime version 建立稳定生成策略。
...
1. Account Runtime IDE primary：`q.{region}.amazonaws.com/generateAssistantResponse`，无 `X-Amz-Target`。
...
`internal/reqconv/cache_points.go` 和本地 `kiro2api` 类似：当 Anthropic tool 带 `cache_control` 时，在 Account Runtime tools 数组里插入：
