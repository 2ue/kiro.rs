# Prompt Cache Scope 开关 + 复现 Account Runtime-RS-Tool 缓存策略 实施方案

> 目标：(1) 给 prompt-cache 命中的 **scope（隔离维度）增加可配置开关**，支持"仅按 session 会话"或"按 client key"隔离；(2) 在当前 `kiro.rs` 上**通过配置 + 最小代码改动复现 `Account Runtime-RS-Tool` 的朴素缓存策略**，且**不影响现有策略**。
>
> **本文档力求自包含**：即使不读源码、不看对话记录，也能据此准确定位、无歧义地实现，不产生误解。

方案时间：2026-06-30。基于对 `kiro.rs` 与 `Account Runtime-RS-Tool` 两个项目缓存实现的逐文件核查。
...
| 项目 | 路径 | 角色 |
|---|---|---|
| `kiro.rs` | `~/Desktop/procode/kiro.rs` | **本方案要改的项目**。Account Runtime→Anthropic 代理，缓存计费是一套四阶段流水线 |
| `Account Runtime-RS-Tool` | `~/Desktop/procode/Account Runtime-RS-Tool` | **参照项目**。同类代理，缓存策略是单文件、无配置的朴素实现 |

两者后端 API 形态相同。`Account Runtime-RS-Tool` 的缓存逻辑全部在单文件 `src/anthropic/cache_metering.rs`。

### 0.2 两套缓存策略的本质关系（关键认知）

**`Account Runtime-RS-Tool` 的缓存 = `kiro.rs` 缓存流水线"第一阶段"裸跑。** `kiro.rs` 在第一阶段（前缀链命中）之上又叠了四层整形。对齐 = 关掉这四层 + 对齐 scope + 解决持久化。

`kiro.rs` 的四阶段流水线：

| 阶段 | 作用 | 对应代码 | Account Runtime-RS-Tool 有无 |
|---|---|---|---|
...
- `Account Runtime-RS-Tool` 只按 **session 或 client-key id** 隔离（二选一作哈希链起头种子），**不绑上游账号** → 跨账号仍命中同一会话前缀。
...
目标：让缓存命中桶可以从"账号+会话+模型+路由"切到"仅会话+模型+路由"，复现 Account Runtime-RS-Tool 的跨账号命中。**照搬 §2.1 的频次控制范式。**
...
pub enum PromptCacheScopeMode {
    ...
    CredentialConversationModel,
    /// 仅会话 + 模型 + 路由。跨账号共享同一会话前缀（Account Runtime-RS-Tool 式）。
    ConversationModel,
...
仅当需要 Account Runtime-RS-Tool 的"session 优先、否则 client key id"语义时做。纯增量穿线，不碰缓存计算逻辑。
...
> 文档作者建议：A + 先上 session。理由：满足"在目标部署复现 Account Runtime-RS-Tool"的诉求，同时严格不触碰其他部署的现有缓存行为。若产品明确要求全局默认即 session 隔离，则选 B 并在变更说明里写明影响面。

---

## 5. 通过配置复现 Account Runtime-RS-Tool 缓存（零代码，配合改动 1）
...
- Account Runtime-RS-Tool：每分钟落盘 `cache_dir/cache_metering.json`，启动读回 → **重启保持命中**。
...
**没有对应配置开关。** 若需要"重启保持命中"，必须给 tracker 加 JSON 落盘 + 启动读回（可直接移植 Account Runtime-RS-Tool `cache_metering.rs` 的落盘段）。`PromptCacheScope` 需可序列化作 map key（它已 `derive` 了所需 trait，但作为 JSON map key 需转成字符串键或落成数组结构 —— 实现时注意）。本项独立于改动 1/3，按需再做。
...
- 本文档讲"缓存 scope 开关 + Account Runtime-RS-Tool 对齐"，是后端 Rust 改动。
