...
2. 重点关注 Account Runtime 账号请求、通过代理系统调用 Account Runtime 上游、Account Runtime 上游响应处理。
3. 明确哪些点适合迁移，哪些点不应照搬。
...
当前 `kiro.rs` 的优势在服务端化、多实例状态、Redis/PgSQL 调度、路径级缓存上报、并发排队、后台统计和配置热更新。外部项目的优势主要在协议细节和客户端兼容经验：它更贴近抓包还原 Account Runtime IDE / Amazon Q CLI 的请求头、端点选择、模型列表拉取、流式事件解析、thinking/tool 转换和错误分类。
...
2. `x-amz-user-agent` / `user-agent` 尽量还原 Account Runtime IDE 或 AmazonQ CLI 的 SDK UA。
...
4. Account Runtime/AWS SDK UA 应继续保留当前项目的配置化方式，不要把外部项目的 `0.12.155` 直接写死。
...
2. 看起来像 Account Runtime/Claude 原生模型 ID 的字符串优先原样透传。
3. 明显不支持的模型才返回 unsupported。
4. 不要把未来 Claude 模型自动降级到旧模型。

第二，外部项目会调用 Account Runtime 上游 `ListAvailableModels`：
...
2. 外部项目解析 `meteringEvent` 并累加 credits。当前项目把 `Metering` 解析为 `()`，没有记录 credits。如果后续需要按 Account Runtime credit 做统计，可以补。
...
## Claude/OpenAI 到 Account Runtime 的协议转换

外部项目 `translator.ts` 有不少值得学习的兼容细节。

### 值得学习的点

1. `cache_control` 只接受 `ephemeral`，映射成 Account Runtime `cachePoint`：`src/main/proxy/translator.ts:60`。
2. 清理 Anthropic billing header，避免污染 Account Runtime prompt：`src/main/proxy/translator.ts:75`。
3. 清理当前 user text 里的 MCP server instructions：`src/main/proxy/translator.ts:80`。
4. OpenAI assistant 历史里的 `reasoning_content` 不传给 Account Runtime，避免 Account Runtime schema 400：`src/main/proxy/translator.ts:577`。
...
9. Account Runtime 返回 `<thinking>...</thinking>` 时拆成 Claude thinking block：`src/main/proxy/translator.ts:1282`。
...
2. 如果当前没有本地 stop sequence 控制，应增加轻量实现；Account Runtime 上游不一定严格遵守 Anthropic `stop_sequences`，代理层兜底更可靠。
3. OpenAI 兼容入口如果后续加入，应复用这些转换规则，特别是不要把 OpenAI `reasoning_content` 直接写进 Account Runtime history。
...
2. 不破坏当前已有特殊业务逻辑。
3. 重点覆盖协议行为，不依赖真实 Account Runtime 网络。
...
当前 `kiro.rs` 已经更像一个可部署的服务端系统；外部项目更像一个桌面客户端增强版，包含大量针对 Account Runtime IDE/CLI 抓包兼容的经验。合理学习方式是把这些经验拆成小功能迁移：
