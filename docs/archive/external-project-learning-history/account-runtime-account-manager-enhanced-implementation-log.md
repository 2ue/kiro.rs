...
## Change 002: 补齐 Account Runtime event-stream 兼容解析
...
- 改动原因：
  - 外部项目显示真实 Account Runtime/Amazon Q 流里可能出现 `messageMetadataEvent.tokenUsage`、`meteringEvent.usage` 和 `codeEvent`。
...
- 详细改动：
  ...
  - 新增 `MeteringEvent { usage }`，保留 Account Runtime credits/usage 信息，当前只做 debug 记录，不参与计费硬逻辑。
...
- 改动后行为：
  - 已知版本和 alias 行为不变。
  - 未来形态的 Claude 原生模型 ID 不再静默降级，直接传给 Account Runtime 上游。
...
## Change 005: 增加 Account Runtime 上游模型能力同步和 /models 合并目录
...
- 改动原因：
  - 外部项目会调用 Account Runtime `ListAvailableModels` 获取真实模型能力和 token limits。
  - 当前项目 `/v1/models` 只有静态硬编码列表，新模型或上下文窗口变化需要发版才能反映。
  - 用户要求结合实际情况优化和能力补齐，并且不能让模型计价/能力影响调度。
- 详细改动：
  - 新增 Account Runtime `ListAvailableModels` 响应类型：
    - `Account RuntimeAvailableModelsResponse`
    - `Account RuntimeAvailableModel`
    - `Account RuntimeModelTokenLimits`
    - `Account RuntimeModelPromptCaching`
  - 扩展 `Account RuntimeEndpoint`：
  ...
  - `IdeEndpoint` 实现 `GET https://q.{region}.amazonaws.com/ListAvailableModels`：
    ...
    - 分页时带 `nextToken`
    - 使用与 Account Runtime IDE 类似的 UA、Authorization、profile header。
  - `Account RuntimeProvider::list_available_models()`：
    - 逐个凭据尝试同步模型能力。
  ...
  - 新增 `ModelCapabilitiesCatalog`：
    - 内置静态保底模型目录。
    - 支持从 Account Runtime 同步结果合并模型能力。
  ...
  - 启动流程新增：
    - 先从 PgSQL 加载模型能力目录。
    - Provider 初始化后异步同步 Account Runtime 上游模型能力。
    - 同步失败只写日志和状态，不阻塞启动、不影响调度。
- 改动前行为：
  - `/models` 返回 handler 内硬编码模型列表。
  - 后台只有模型价格同步，没有 Account Runtime 模型能力同步。
  - 新模型发布后只能依赖代码更新。
- 改动后行为：
  - `/models` 总是有静态保底。
  - 如果 Account Runtime `ListAvailableModels` 同步成功，`/models` 会包含同步到的新模型。
  - 后台可查看和手动同步模型能力。
...
- 详细改动：
  - `Account RuntimeProvider::list_available_models()` 改为从 `token_manager.snapshot().entries` 读取实际凭据 ID。
  - 只遍历未禁用凭据；这是因为模型能力同步会真实调用 Account Runtime 上游，不能绕过用户手动禁用。后台列表、导入查重、分组等管理类能力仍应使用所有未软删除凭据。
...
## Change 008: 非流式响应同步补齐新增 Account Runtime 事件兼容
...
- 改动后行为：
  - 流式和非流式对这三类 Account Runtime 事件的语义一致。
