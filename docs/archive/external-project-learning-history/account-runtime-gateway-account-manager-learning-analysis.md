...
- 最近已经吸收过一部分 Account Runtime-Go / 同类项目中的协议兼容点，例如 tool schema 递归清洗、tool name 映射、模型映射和日志记录。

但从 `../kiro-gateway` 和 `../kiro-account-manager` 还能继续学习几个高价值方向：

1. 请求打到 Account Runtime 上游前的 payload 大小防御。
...
- 不支持模型时前置返回 400，而不是盲目打到 Account Runtime 上游。
...
当前项目已经在 `src/anthropic/converter.rs` 中处理了 Account Runtime 对工具定义的敏感点：

- 递归移除 `additionalProperties`。
- 移除非法或空 `required`。
- 修复根 schema 缺失 `type/properties`。
- 将 `_`、`-`、非 ASCII 字母数字分隔符转换为 Account Runtime-safe 的 camelCase 风格名称。
- 超长 tool name 使用确定性 hash 后缀缩短。
- 保留 short -> original 的映射，用于响应中还原工具名称。

这部分已经吸收了 `Account Runtime-Go`/同类实现的核心思想，且当前实现比简单截断更安全，因为能避免 `foo-bar` 与 `foo_bar` 这类映射碰撞。
...
`../kiro-gateway` 是 Python/FastAPI 风格的网关项目。它的强项不在生产级调度，而在 Account Runtime 协议兼容、防止上游 400、请求转换前防御。
...
- Account Runtime 上游在 payload 超大时可能返回误导性的 `400 Improperly formed request`。
- 所以在发送前先计算 JSON 序列化后的字节数。
- 超限时裁剪最旧 history。
- 裁剪后确保 history 起点是 userInputMessage。
- 裁剪后修复孤立 toolResults。
- 移除空的 `toolUses: []`，因为这也是 Account Runtime 敏感点。

参考实现：

- `kiro/payload_guards.py:20-28`：说明 Account Runtime 超大 payload 会返回 misleading 400。
...
- 但没有看到一个统一的“最终 Account Runtime payload 序列化字节数 preflight + 自动裁剪”模块。

建议：

优先吸收这个思想，在 `kiro.rs` 中新增最终 payload guard，位置建议在转换完成之后、调用 `Account RuntimeProvider` 之前。

设计要求：

- 对最终发给 Account Runtime 的 JSON payload 做字节级测量，不只看 token。
...
- 都是在 Anthropic 请求转换成 `Account RuntimeRequest` 后、调用 `Account RuntimeProvider` 前执行。

已实现能力：

- 按最终 Account Runtime JSON request body 的真实字节数计算 payload 大小。
...
- 仍超限时前置返回下游 `400 invalid_request_error`，不再浪费账号打 Account Runtime 上游。
- 当启用 debug/告警头时，把裁剪和修复动作合并进 `x-account-runtime-warnings`。
...
- 如果 tool_result 找不到前面的 tool_use，不直接让请求进入 Account Runtime。
- 也不是简单丢弃。
...
- 孤立 tool_result 不作为 Account Runtime toolResult 发送。
- 但将其内容追加为普通文本，例如：
...
- 但当前请求没有 tools 定义。
- Account Runtime 上游可能因为历史引用了未知工具而 400。
...
- 这可以解决一部分 Account Runtime 要求“历史工具必须有定义”的问题。
...
不要完全照搬。当前项目应该按 Account Runtime 上游实际语义拆分：
...
- 502/503/504：
  - 如果是 Account Runtime 全局服务异常，短暂全局 backoff。
...
- 常量 `MAX_ACCOUNT_RUNTIME_PAYLOAD_SIZE: 450 * 1024`。
- 构建 Account Runtime payload 后再做 JSON value 级别裁剪。
- 超限后循环裁剪历史。
- 优先保留最近消息。
...
这个实现中的 JSON 字段名检查疑似使用了 snake_case，例如 `assistant_response_message`、`tool_uses`，而实际 Account Runtime payload 序列化常见是 camelCase，例如 `assistantResponseMessage`、`toolUses`。如果直接照搬，可能导致 tool 配对保护失效。
...
- 用户之前已经明确指出：Account Runtime 官方接口查出来的是 credits，不是美元。
...
- 下游模型可能不固定。
- Account Runtime 上游模型可能不固定。
- 需要尽可能把下游模型映射到 Account Runtime 支持模型，减少 400。
- 日志里记录模型转换链路。
...
### 7.1 P0：最终 Account Runtime payload 字节级 guard

这是本次最重要的建议。

原因：

- Account Runtime 上游的 `Improperly formed request` 不一定是 schema 错，也可能是 payload 超大。
- 单靠 token 估算不能覆盖 JSON 序列化体积、base64 image/document、tool schema 巨大、历史 tool result 巨大等场景。

建议实现：

1. 在转换完成后，对最终 Account Runtime 请求体序列化。
2. 计算字节数。
...
- 孤立 tool_result 不进入 Account Runtime toolResults。
...
1. 实现最终 Account Runtime payload byte guard。
